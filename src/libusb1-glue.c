...
static int find_interface_and_endpoints(libusb_device *dev, uint8_t *conf,
                                        uint8_t *interface, uint8_t *altsetting,
                                        int *inep, int *inep_maxpacket,
                                        int *outep, int *outep_maxpacket,
                                        int *intep);
static void clear_stall(PTP_USB *ptp_usb);
static int init_ptp_usb(PTPParams *params, PTP_USB *ptp_usb,
                        libusb_device *dev);
...
static int probe_device_descriptor(libusb_device *dev, FILE *dumpfile) {
    ...
    for (i = 0; i < desc.bNumConfigurations; i++) {
        ...
        if (ret != LIBUSB_SUCCESS) {
            LIBMTP_INFO("configdescriptor %d get failed with ret %d in "
                        "probe_device_descriptor yet "
 ...
 */
	    ...
	    if (was_mtp_extension) {
	      ...
	      return 1;
                ...
                ret = libusb_get_string_descriptor_ascii(
                    devh, config->interface[j].altsetting[k].iInterface, buf,
                    1024);
                if (ret < 3)
                    continue;
                if (strstr((char *)buf, "MTP") != NULL) {
                    if (dumpfile != NULL) {
                        fprintf(
                            dumpfile,
                            "Configuration %d, interface %d, altsetting %d:\n",
                            i, j, k);
                        fprintf(dumpfile, "   Interface description contains "
                                          "the string \"MTP\"\n");
                        fprintf(dumpfile, "   Device recognized as MTP, no "
                                          "further probing.\n");
                    }
                ...
                if (libusb_kernel_driver_active(
                        devh, config->interface[j].altsetting[k].iInterface)) {
                    /*
                     * Specifically avoid probing anything else than USB mass
                     * storage devices
                     * and non-associated drivers in Linux.
                     */
                    if (config->interface[j].altsetting[k].bInterfaceClass !=
                        LIBUSB_CLASS_MASS_STORAGE) {
                        LIBMTP_INFO("avoid probing device using attached "
                                    "kernel interface\n");
                        libusb_free_config_descriptor(config);
                        libusb_close(devh);
                        return 0;
    ...
    if (desc.bDeviceClass == LIBUSB_CLASS_VENDOR_SPEC ||
        ...
        ret = libusb_control_transfer(
            devh, LIBUSB_ENDPOINT_IN | LIBUSB_RECIPIENT_DEVICE |
                      LIBUSB_REQUEST_TYPE_VENDOR,
            cmd, 0, 4, buf, sizeof(buf), USB_TIMEOUT_DEFAULT);

        // Dump it, if requested
        if (dumpfile != NULL && ret > 0) {
            fprintf(
                dumpfile,
                "Microsoft device response to control message 1, CMD 0x%02x:\n",
                cmd);
            data_dump_ascii(dumpfile, buf, ret, 16);
        }
        ...
        if (ret <= 0x15) {
            /* TODO: If there was an error, flag it and let the user know
             * somehow */
            /* if(ret == -1) {} */
            libusb_close(devh);
            return 0;
        ...
        ret = libusb_control_transfer(
            devh, LIBUSB_ENDPOINT_IN | LIBUSB_RECIPIENT_DEVICE |
                      LIBUSB_REQUEST_TYPE_VENDOR,
            cmd, 0, 5, buf, sizeof(buf), USB_TIMEOUT_DEFAULT);

        // Dump it, if requested
        if (dumpfile != NULL && ret > 0) {
            fprintf(
                dumpfile,
                "Microsoft device response to control message 2, CMD 0x%02x:\n",
                cmd);
            data_dump_ascii(dumpfile, buf, ret, 16);
        }
        ...
        if (ret == -1) {
            /* TODO: Implement callback function to let managing program know
               there
            ...
            fprintf(dumpfile,
                    "Zero-length response to control message 2 (OK)\n");
...
get_mtp_usb_device_list(mtpdevice_list_t **mtp_device_list) {
    ssize_t nrofdevs;
        ...
        if (desc.bDeviceClass != LIBUSB_CLASS_HUB) {
            ...
            for (i = 0; i < mtp_device_table_size; i++) {
                if (desc.idVendor == mtp_device_table[i].vendor_id &&
                    ...
                    *mtp_device_list = append_to_mtpdevice_list(
                        *mtp_device_list, dev, libusb_get_bus_number(dev));
                    found = 1;
                    break;
                }
            }
            ...
            if (!found) {
                ...
                /*
                 * By thomas_-_s: Also append devices that are no MTP but PTP
                 * devices
                ...
                LIBUSB_CLASS_PTP && dev->descriptor.bDeviceClass !=
                LIBUSB_CLASS_HUB ) {
                    *mtp_device_list =
                append_to_mtpdevice_list(*mtp_device_list, dev, bus->location);
                  }
                }
                */
            }
...
static short ptp_read_func(unsigned long size, PTPDataHandler *handler,
                           void *data, unsigned long *readbytes, int readzero) {
    ...
    while (curread < size) {
        ...
        ret = USB_BULK_READ(ptp_usb->handle, ptp_usb->inep, bytes, toread,
                            &xread, ptp_usb->timeout);
        ...
        int putfunc_ret =
            handler->putfunc(NULL, handler->priv, xread, bytes, &written);
        ...
        if (ptp_usb->callback_active) {
            if (ptp_usb->current_transfer_complete >=
                ...
                ptp_usb->current_transfer_complete =
                    ptp_usb->current_transfer_total;
                ptp_usb->callback_active = 0;
            }
            if (ptp_usb->current_transfer_callback != NULL) {
                int ret;
                ret = ptp_usb->current_transfer_callback(
                    ptp_usb->current_transfer_complete,
                    ptp_usb->current_transfer_total,
    ...
    if (readzero && !FLAG_NO_ZERO_READS(ptp_usb) &&
        ...
        zeroresult = USB_BULK_READ(ptp_usb->handle, ptp_usb->inep, &temp, 0,
                                   &xread, ptp_usb->timeout);
        if (zeroresult != LIBUSB_SUCCESS)
            LIBMTP_INFO(
                "LIBMTP panic: unable to read in zero packet, response 0x%04x",
                zeroresult);
    }
...
static short ptp_write_func(unsigned long size, PTPDataHandler *handler,
                            void *data, unsigned long *written) {
    PTP_USB *ptp_usb = (PTP_USB *)data;
    ...
    while (curwrite < size) {
        ...
        if (towrite > CONTEXT_BLOCK_SIZE) {
            ...
            if (towrite > ptp_usb->outep_maxpacket &&
                towrite % ptp_usb->outep_maxpacket != 0) {
                towrite -= towrite % ptp_usb->outep_maxpacket;
            }
        }
        int getfunc_ret =
            handler->getfunc(NULL, handler->priv, towrite, bytes, &towrite);
        if (getfunc_ret != PTP_RC_OK) {
            free(bytes);
            return getfunc_ret;
        }
        while (usbwritten < towrite) {
            ret = USB_BULK_WRITE(ptp_usb->handle, ptp_usb->outep,
                                 bytes + usbwritten, towrite - usbwritten,
                                 &xwritten, ptp_usb->timeout);
...
uint16_t ptp_usb_sendreq(PTPParams *params, PTPContainer *req) {
    ...
    usbreq.length =
        htod32(PTP_USB_BULK_REQ_LEN - (sizeof(uint32_t) * (5 - req->Nparam)));
    ...
    if (written != towrite && ret != PTP_ERROR_CANCEL && ret != PTP_ERROR_IO) {
        libusb_glue_error(params, "PTP: request code 0x%04x sending req wrote "
                                  "only %ld bytes instead of %d",
...
uint16_t ptp_usb_senddata(PTPParams *params, PTPContainer *ptp, uint64_t size,
    ...
    ((PTP_USB *)params->data)->current_transfer_total =
        size + PTP_USB_BULK_HDR_LEN;
    ...
    if (params->split_header_data) {
        ...
        datawlen = (size < PTP_USB_BULK_PAYLOAD_LEN_WRITE)
                       ? size
    ...
    do {
        ...
        if (dtoh16(usbdata.code) != ptp->Code) {
            if (FLAG_IGNORE_HEADER_ERRORS(ptp_usb)) {
                libusb_glue_debug(params,
                                  ...
                                  "PTP header, code field insane, expect "
                                  "problems! (But continuing)");
                // Repair the header, so it won't wreak more havoc, don't just
                // ignore it.
                ...
                // makes it possible to return error codes in the code field
                // when
                // getting data. It appears Windows ignores the contents of this
                // field entirely.
                if (ret < PTP_RC_Undefined ||
                    ret > PTP_RC_SpecificationOfDestinationUnsupported) {
                    libusb_glue_debug(params,
                                      "ptp2/ptp_usb_getdata: detected a broken "
        ...
        if (rlen == PTP_USB_BULK_HS_MAX_PACKET_LEN_READ) {
            ...
            while (1) {
                ...
                xret = ptp_read_func(0x20000000, handler, params->data,
                                     &readdata, 0);
                if (xret != PTP_RC_OK)
                    return xret;
        ...
        /* Copy first part of data to 'data' */
        putfunc_ret =
            handler->putfunc(params, handler->priv, rlen - PTP_USB_BULK_HDR_LEN,
                             usbdata.payload.data, &written);
        if (putfunc_ret != PTP_RC_OK)
            return putfunc_ret;
        ...
        if (FLAG_NO_ZERO_READS(ptp_usb) &&
            ...
            result = USB_BULK_READ(ptp_usb->handle, ptp_usb->inep, &byte, 1,
                                   &xread, ptp_usb->timeout);

            if (result != 1)
                LIBMTP_INFO("Could not read in extra byte for "
                            "PTP_USB_BULK_HS_MAX_PACKET_LEN_READ long file, "
                            "return value 0x%04x\n",
                            result);
            ...
            zeroresult = USB_BULK_READ(ptp_usb->handle, ptp_usb->inep,
                                       &zerobyte, 0, &xread, ptp_usb->timeout);

            if (zeroresult != 0)
                LIBMTP_INFO("LIBMTP panic: unable to read in zero packet, "
                            "response 0x%04x",
                            zeroresult);
        }
        ...
        ret = ptp_read_func(len - (rlen - PTP_USB_BULK_HDR_LEN), handler,
                            params->data, &rlen, 1);
...
uint16_t ptp_usb_getresp(PTPParams *params, PTPContainer *resp) {
    ...
    // sends a packet containing just zeroes of length 2 (up to 4 has been seen
    // too)
    // after a NULL packet when it should send the response. This code ignores
    // such illegal packets.
    while (ret == PTP_RC_OK && rlen < PTP_USB_BULK_HDR_LEN &&
           usbresp.length == 0) {
    ...
    if (FLAG_IGNORE_HEADER_ERRORS(ptp_usb)) {
        if (resp->Transaction_ID != params->transaction_id - 1) {
            libusb_glue_debug(params,
                              "ptp_usb_getresp: detected a broken "
...
static inline uint16_t ptp_usb_event(PTPParams *params, PTPContainer *event,
    ...
    if (rlen < 8) {
        libusb_glue_error(
            params, "PTP: reading event an short read of %ld bytes occurred",
            rlen);
        return PTP_ERROR_IO;
    }
...
uint16_t ptp_usb_control_cancel_request(PTPParams *params,
    ...
    ret = libusb_control_transfer(
        ptp_usb->handle, LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE,
        0x64, 0x0000, 0x0000, buffer, sizeof(buffer), ptp_usb->timeout);
...
static int init_ptp_usb(PTPParams *params, PTP_USB *ptp_usb,
    ...
    if (FLAG_UNLOAD_DRIVER(ptp_usb) &&
        ...
        if (LIBUSB_SUCCESS !=
            ...
            perror(
                "libusb_detach_kernel_driver() failed, continuing anyway...");
        }
    }
    ...
    if (config->bConfigurationValue != ptp_usb->config) {
        fprintf(stderr, "desired configuration different from current, trying "
 ...
 */
  ...
  if (config->interface[].altsetting[].bAlternateSetting !=
    ...
    if (FLAG_SWITCH_MODE_BLACKBERRY(ptp_usb)) {
        ...
        usleep(1000);
        ret = libusb_control_transfer(
            device_handle, LIBUSB_REQUEST_TYPE_VENDOR |
...
static void close_usb(PTP_USB *ptp_usb) {
    if (!FLAG_NO_RELEASE_INTERFACE(ptp_usb)) {
         ...
         * (...) Windows, when clearing a stall, only sends the CLEAR_HALT
         * command,
         * and presumes that the stall has cleared.  Some devices actually choke
         * if the CLEAR_HALT is followed by a GET_STATUS (used to determine if
         * the
...
LIBMTP_error_number_t configure_usb_device(LIBMTP_raw_device_t *device,
    ...
    err = find_interface_and_endpoints(
        ldevice, &ptp_usb->config, &ptp_usb->interface, &ptp_usb->altsetting,
        &ptp_usb->inep, &ptp_usb->inep_maxpacket, &ptp_usb->outep,
        &ptp_usb->outep_maxpacket, &ptp_usb->intep);
    ...
    if ((ret = ptp_opensession(params, 1)) == PTP_ERROR_IO) {
        LIBMTP_ERROR("PTP_ERROR_IO: failed to open session, trying again after "
                     "resetting USB interface\n");
    ...
    if (ret == PTP_RC_InvalidTransactionID) {
        LIBMTP_ERROR("LIBMTP WARNING: Transaction ID was invalid, increment "
                     "and try again\n");
...
static int usb_get_endpoint_status(PTP_USB *ptp_usb, int ep, uint16_t *status) {
    return libusb_control_transfer(
        ptp_usb->handle, LIBUSB_ENDPOINT_IN | LIBUSB_RECIPIENT_ENDPOINT,
        LIBUSB_REQUEST_GET_STATUS, USB_FEATURE_HALT, ep,
        (unsigned char *)status, 2, ptp_usb->timeout);
}
