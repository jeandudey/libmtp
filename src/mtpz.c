...
int mtpz_loaddata() {
    ...
    // Should only be six characters in length, but fgets will encounter a
    // newline and stop.
    MTPZ_PUBLIC_EXPONENT =
        (unsigned char *)fgets_strip((char *)malloc(8), 8, fdata);
    if (!MTPZ_PUBLIC_EXPONENT) {
        LIBMTP_ERROR("Unable to read MTPZ public exponent from ~/.mtpz-data, "
                     "MTPZ disabled.\n");
        return -1;
    }

    // Should only be 33 characters in length, but fgets will encounter a
    // newline and stop.
    char *hexenckey =
        (unsigned char *)fgets_strip((char *)malloc(35), 35, fdata);
    if (!hexenckey) {
        LIBMTP_ERROR("Unable to read MTPZ encryption key from ~/.mtpz-data, "
                     "MTPZ disabled.\n");
        return -1;
    }
    ...
    // newline and stop.
    MTPZ_MODULUS =
        (unsigned char *)fgets_strip((char *)malloc(260), 260, fdata);
    if (!MTPZ_MODULUS) {
        LIBMTP_ERROR(
            "Unable to read MTPZ modulus from ~/.mtpz-data, MTPZ disabled.\n");
        return -1;
    }
    ...
    MTPZ_PRIVATE_KEY =
        (unsigned char *)fgets_strip((char *)malloc(260), 260, fdata);
    if (!MTPZ_PRIVATE_KEY) {
        LIBMTP_ERROR("Unable to read MTPZ private key from ~/.mtpz-data, MTPZ "
                     "disabled.\n");
        return -1;
    }

    // Should only be 1258 characters in length, but fgets will encounter the
    // end of the file and stop.
    char *hexcerts = fgets_strip((char *)malloc(1260), 1260, fdata);
    if (!hexcerts) {
        LIBMTP_ERROR("Unable to read MTPZ certificates from ~/.mtpz-data, MTPZ "
                     "disabled.\n");
        return -1;
    }
    MTPZ_CERTIFICATES = hex_to_bytes(hexcerts, strlen(hexcerts));
    if (!MTPZ_CERTIFICATES) {
        LIBMTP_ERROR("Unable to parse MTPZ certificates from ~/.mtpz-data, "
                     "MTPZ disabled.\n");
        return -1;
    }
...
mtpz_rsa_t *mtpz_rsa_init(const unsigned char *modulus,
                          const unsigned char *priv_key,
                          const unsigned char *pub_exp);
void mtpz_rsa_free(mtpz_rsa_t *);
int mtpz_rsa_decrypt(int flen, unsigned char *from, int tlen, unsigned char *to,
                     mtpz_rsa_t *rsa);
int mtpz_rsa_sign(int flen, unsigned char *from, int tlen, unsigned char *to,
                  mtpz_rsa_t *rsa);
...
static unsigned int mtpz_hash_f(int s, unsigned int x, unsigned int y,
                                unsigned int z);
...
void mtpz_encryption_cipher(unsigned char *data, unsigned int len,
                            char encrypt);
void mtpz_encryption_cipher_advanced(unsigned char *key, unsigned int key_len,
                                     unsigned char *data, unsigned int data_len,
                                     char encrypt);
unsigned char *mtpz_encryption_expand_key(unsigned char *constant, int key_len,
                                          int count, int *out_len);
void mtpz_encryption_expand_key_inner(unsigned char *constant, int key_len,
                                      unsigned char **out, int *out_len);
void mtpz_encryption_inv_mix_columns(unsigned char *expanded, int offset,
                                     int rounds);
void mtpz_encryption_decrypt_custom(unsigned char *data, unsigned char *seed,
                                    unsigned char *expanded);
void mtpz_encryption_encrypt_custom(unsigned char *data, unsigned char *seed,
                                    unsigned char *expanded);
void mtpz_encryption_encrypt_mac(unsigned char *hash, unsigned int hash_length,
...
mtpz_rsa_t *mtpz_rsa_init(const unsigned char *str_modulus,
                          const unsigned char *str_privkey,
                          const unsigned char *str_pubexp) {
    ...
    gcry_sexp_build(&rsa->privkey, NULL,
                    "(private-key (rsa (n %m) (e %m) (d %m)))", mpi_modulus,
                    mpi_pubexp, mpi_privkey);
    gcry_sexp_build(&rsa->pubkey, NULL, "(public-key (rsa (n %m) (e %m)))",
...
int mtpz_rsa_decrypt(int flen, unsigned char *from, int tlen, unsigned char *to,
    ...
    gcry_sexp_build(&sexp_data, NULL, "(enc-val (flags raw) (rsa (a %m)))",
                    mpi_from);
    ...
    // Lame workaround. GCRYMPI_FMT_USG gets rid of any leading zeroes which we
    // do need,
    // so we'll count how many bits are being used, and subtract that from how
    // many bits actually
    // should be there, and then write into our output array shifted over
    // however many bits/8.
...
void mtpz_hash_finalize_hash(char *state, char *out) {
    ...
    v5[0] = '\x80';
    v6 =
        8 * state_box[MTPZ_HASHSTATE_84] | (state_box[MTPZ_HASHSTATE_88] >> 29);
...
void mtpz_hash_compute_hash(char *state, char *msg, int len) {
    ...
    for (i = 0; i < 16; i++)
        W[i] = MTPZ_SWAP(M[i]);
    for (i = 16; i < 80; i++)
        W[i] = mtpz_hash_rotate_left(
            W[i - 3] ^ W[i - 8] ^ W[i - 14] ^ W[i - 16], 1);

    // 2 - initialize five working variables a, b, c, d, e with previous hash
    // value
    a = state_box[0];
    b = state_box[1];
    c = state_box[2];
    ...
    for (i = 0; i < 80; i++) {
        s = i / 20;
        T = (mtpz_hash_rotate_left(a, 5) + mtpz_hash_f(s, b, c, d) + e + K[s] +
             W[i]) &
            0xFFFFFFFF;
        e = d;
        d = c;
...
unsigned int mtpz_hash_f(int s, unsigned int x, unsigned int y,
                         unsigned int z) {
    switch (s) {
    case 0:
        return (x & y) ^ (~x & z); // Ch()
...
void mtpz_encryption_cipher(unsigned char *data, unsigned int len,
    ...
    if ((count & 0x0F) == 0) {
        int exp_len = 0;
        expanded = mtpz_encryption_expand_key(
            (unsigned char *)MTPZ_ENCRYPTION_KEY, 16, 10, &exp_len);

        if (count != 0) {
            do {
                if (encrypt)
                    mtpz_encryption_encrypt_custom(data + offset, NULL,
                                                   expanded);
                else
                    mtpz_encryption_decrypt_custom(data + offset, NULL,
                                                   expanded);
...
void mtpz_encryption_cipher_advanced(unsigned char *key, unsigned int key_len,
                                     unsigned char *data, unsigned int data_len,
                                     char encrypt) {
    int len = (key_len == 16) ? 10 : (key_len == 24) ? 12 : 32;
    int exp_len;
    unsigned char *expanded =
        mtpz_encryption_expand_key(key, key_len, len, &exp_len);
...
void mtpz_encryption_expand_key_inner(unsigned char *constant, int key_len,
    ...
    for (i = key_len; i < ks; i += 4) {
        ...
        for (j = 0; j < 4; j++) {
            key[i + j] =
                (unsigned char)((key[i + j - key_len] ^ temp[j]) & 0xFF);
        }
    }
...
void mtpz_encryption_inv_mix_columns(unsigned char *expanded, int offset,
    ...
    for (v8 = 1; v8 < rounds; v8++) {
        ...
        exp_int[0] = MTPZ_SWAP(
            mtpz_aes_gb9[expanded[o + 19]] ^ mtpz_aes_gb13[expanded[o + 18]] ^
            mtpz_aes_gb11[expanded[o + 17]] ^ mtpz_aes_gb14[expanded[o + 16]]);
        exp_int[1] = MTPZ_SWAP(
            mtpz_aes_gb9[expanded[o + 23]] ^ mtpz_aes_gb13[expanded[o + 22]] ^
            mtpz_aes_gb11[expanded[o + 21]] ^ mtpz_aes_gb14[expanded[o + 20]]);
        exp_int[2] = MTPZ_SWAP(
            mtpz_aes_gb9[expanded[o + 27]] ^ mtpz_aes_gb13[expanded[o + 26]] ^
            mtpz_aes_gb11[expanded[o + 25]] ^ mtpz_aes_gb14[expanded[o + 24]]);
        exp_int[3] = MTPZ_SWAP(
            mtpz_aes_gb9[expanded[o + 31]] ^ mtpz_aes_gb13[expanded[o + 30]] ^
            mtpz_aes_gb11[expanded[o + 29]] ^ mtpz_aes_gb14[expanded[o + 28]]);
        o += 16;
    }
}
...
void mtpz_encryption_decrypt_custom(unsigned char *data, unsigned char *seed,
    ...
    unsigned int v14 =
        MTPZ_SWAP(u_seed[0]) ^ MTPZ_SWAP(u_expanded[(keyOffset) / 4]);
    unsigned int v15 =
        MTPZ_SWAP(u_seed[1]) ^ MTPZ_SWAP(u_expanded[(keyOffset + 4) / 4]);
    unsigned int v16 =
        MTPZ_SWAP(u_seed[2]) ^ MTPZ_SWAP(u_expanded[(keyOffset + 8) / 4]);
    unsigned int v17 =
        MTPZ_SWAP(u_seed[3]) ^ MTPZ_SWAP(u_expanded[(keyOffset + 12) / 4]);

    unsigned int v18 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v15)] ^
                       mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v16)] ^
                       mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v14)] ^
                       mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v17)];
    unsigned int v19 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v16)] ^
                       mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v17)] ^
                       mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v15)] ^
                       mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v14)];
    unsigned int v20 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v17)] ^
                       mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v14)] ^
                       mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v16)] ^
                       mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v15)];
    unsigned int v21 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v14)] ^
                       mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v15)] ^
                       mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v17)] ^
                       mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v16)];
    ...
    do {
        ...
        v18 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v15)] ^
              mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v16)] ^
              mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v14)] ^
              mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v17)];
        v19 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v16)] ^
              mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v17)] ^
              mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v15)] ^
              mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v14)];
        v20 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v17)] ^
              mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v14)] ^
              mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v16)] ^
              mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v15)];
        v21 = mtpz_aes_rt1[MTPZ_ENCRYPTIONBYTE3(v14)] ^
              mtpz_aes_rt2[MTPZ_ENCRYPTIONBYTE2(v15)] ^
              mtpz_aes_rt3[MTPZ_ENCRYPTIONLOBYTE(v17)] ^
              mtpz_aes_rt4[MTPZ_ENCRYPTIONBYTE1(v16)];
...
void mtpz_encryption_encrypt_custom(unsigned char *data, unsigned char *seed,
    ...
    unsigned int v18 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v17)] ^
                       mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v16)] ^
                       mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v14)] ^
                       mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v15)];
    unsigned int v19 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v14)] ^
                       mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v17)] ^
                       mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v15)] ^
                       mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v16)];
    unsigned int v20 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v15)] ^
                       mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v14)] ^
                       mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v16)] ^
                       mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v17)];
    unsigned int v21 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v16)] ^
                       mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v15)] ^
                       mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v17)] ^
                       mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v14)];
    ...
    do {
        ...
        v18 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v17)] ^
              mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v16)] ^
              mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v14)] ^
              mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v15)];
        v19 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v14)] ^
              mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v17)] ^
              mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v15)] ^
              mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v16)];
        v20 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v15)] ^
              mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v14)] ^
              mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v16)] ^
              mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v17)];
        v21 = mtpz_aes_ft1[MTPZ_ENCRYPTIONBYTE3(v16)] ^
              mtpz_aes_ft2[MTPZ_ENCRYPTIONBYTE2(v15)] ^
              mtpz_aes_ft3[MTPZ_ENCRYPTIONLOBYTE(v17)] ^
              mtpz_aes_ft4[MTPZ_ENCRYPTIONBYTE1(v14)];
...
void mtpz_encryption_encrypt_mac(unsigned char *hash, unsigned int hash_length,
...

    {
        ...
        for (i = 0; i < 16; i++)
            loop1[i] =
                (unsigned char)((2 * enc_hash[i]) | (enc_hash[i + 1] >> 7));
    ...
    {
        int len = (hash_length == 16) ? 10 : (hash_length == 24) ? 12 : 32;
        int exp_len;
        unsigned char *expanded =
            mtpz_encryption_expand_key(hash, hash_length, len, &exp_len);
...
/* Corresponds to Rcon[30] (seems to be truncated to include only the used
 * constants) */
unsigned char mtpz_aes_rcon[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80,
...
static uint16_t
ptp_mtpz_validatehandshakeresponse(PTPParams *params, unsigned char *random,
    ...
    if (ret == PTP_RC_OK) {
        ...
        mtpz_rsa_t *rsa =
            mtpz_rsa_init(MTPZ_MODULUS, MTPZ_PRIVATE_KEY, MTPZ_PUBLIC_EXPONENT);
        if (!rsa) {
        ...
        if (mtpz_rsa_decrypt(128, (unsigned char *)message, 128,
                             (unsigned char *)msg_dec, rsa) == 0) {
        ...
        mtpz_encryption_cipher_advanced((unsigned char *)hash_key, 16, act_msg,
                                        832, 0);
        ...
        unsigned int rand_length =
            MTPZ_SWAP(*(unsigned short *)(act_reader) << 16);
        act_reader += 2;
...
static uint16_t ptp_mtpz_opensecuresyncsession(PTPParams *params,
                                               unsigned char *hash) {
    ...
    ret = ptp_mtpz_wmdrmpd_enabletrustedfilesoperations(
        params, MTPZ_SWAP(hashparams[0]), MTPZ_SWAP(hashparams[1]),
        MTPZ_SWAP(hashparams[2]), MTPZ_SWAP(hashparams[3]));
    return ret;
};

static unsigned char *
ptp_mtpz_makeapplicationcertificatemessage(unsigned int *out_len,
                                           unsigned char **out_random) {
    *out_len = 785;
    ...
    // Write the marker bytes, length of certificates, and certificates
    // themselves.
    *(target++) = '\x02';
    *(target++) = '\x01';
    *(target++) = '\x01';
    ...
    *out_random = random;
    ...
    mtpz_rsa_sign(128, (unsigned char *)odata, 128, (unsigned char *)signature,
                  rsa);
...
static unsigned char *ptp_mtpz_makeconfirmationmessage(unsigned char *hash,
...
uint16_t ptp_mtpz_handshake(PTPParams *params) {
    ...
    ret = ptp_setdevicepropvalue(params, PTP_DPC_MTP_SessionInitiatorInfo,
                                 &propval, PTP_DTC_STR);
    if (ret != PTP_RC_OK)
        return ret;
    ...
    applicationCertificateMessage =
        ptp_mtpz_makeapplicationcertificatemessage(&size, &random);
    ret = ptp_mtpz_sendwmdrmpdapprequest(params, applicationCertificateMessage,
                                         size);
