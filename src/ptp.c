...
static uint16_t ptp_exit_recv_memory_handler(PTPDataHandler *, unsigned char **,
                                             unsigned long *);
static uint16_t ptp_init_recv_memory_handler(PTPDataHandler *);
static uint16_t ptp_init_send_memory_handler(PTPDataHandler *, unsigned char *,
                                             unsigned long len);
...
uint16_t ptp_transaction_new(PTPParams *params, PTPContainer *ptp,
    ...
    while (tries--) {
        ...
        if (ptp->Transaction_ID < params->transaction_id - 1) {
            tries++;
            ptp_debug(params, "PTP: Sequence number mismatch %d vs expected "
                              "%d, suspecting old reply.",
...
static uint16_t memory_getfunc(PTPParams *params, void *private,
...
static uint16_t memory_putfunc(PTPParams *params, void *private,
...
static uint16_t ptp_init_recv_memory_handler(PTPDataHandler *handler) {
 ...
 */
static uint16_t ptp_init_send_memory_handler(PTPDataHandler *handler,
                                             unsigned char *data,
                                             unsigned long len) {
    PTPMemHandlerPrivate *priv;
    priv = malloc(sizeof(PTPMemHandlerPrivate));
...
static uint16_t ptp_exit_recv_memory_handler(PTPDataHandler *handler,
                                             unsigned char **data,
                                             unsigned long *size) {
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate *)handler->priv;
    *data = priv->data;
...
typedef struct { int fd; } PTPFDHandlerPrivate;

static uint16_t fd_getfunc(PTPParams *params, void *private,
                           unsigned long wantlen, unsigned char *data,
                           unsigned long *gotlen) {
...
static uint16_t fd_putfunc(PTPParams *params, void *private,
                           unsigned long sendlen, unsigned char *data,
                           unsigned long *putlen) {
    int written;
    PTPFDHandlerPrivate *priv = (PTPFDHandlerPrivate *)private;
...
static uint16_t ptp_init_fd_handler(PTPDataHandler *handler, int fd) {
...
static uint16_t ptp_exit_fd_handler(PTPDataHandler *handler) {
...
uint16_t ptp_transaction(PTPParams *params, PTPContainer *ptp, uint16_t flags,
                         uint64_t sendlen, unsigned char **data,
...
uint16_t ptp_canon_eos_getdeviceinfo(PTPParams *params,
                                     PTPCanonEOSDeviceInfo *di) {
    uint16_t ret;
    PTPContainer ptp;
    PTPDataHandler handler;
    unsigned long len;
...
#ifdef HAVE_LIBXML2
static int traverse_tree(PTPParams *params, int depth, xmlNodePtr node) {
    xmlNodePtr next;
    xmlChar *xchar;
    int n;
    char *xx;
...
static int parse_9301_cmd_tree(PTPParams *params, xmlNodePtr node,
                               PTPDeviceInfo *di) {
    xmlNodePtr next;
    int cnt;
...
static int parse_9301_value(PTPParams *params, const char *str, uint16_t type,
...
static int parse_9301_propdesc(PTPParams *params, xmlNodePtr next,
    ...
    do {
        ...
        if (!strcmp((char *)next->name,
                    "default")) { /* propdesc.FactoryDefaultValue */
            ptp_debug(params, "default value");
            parse_9301_value(params, (char *)xmlNodeGetContent(next), type,
        ...
        if (!strcmp((char *)next->name, "enum")) { /* propdesc.FORM.Enum */
            ...
            dpd->FORM.Enum.SupportedValue =
                malloc(n * sizeof(PTPPropertyValue));
            s = (char *)xmlNodeGetContent(next);
            i = 0;
            do {
                parse_9301_value(
                    params, s, type,
                    &dpd->FORM.Enum
                         .SupportedValue[i]); /* should turn ' ' into \0? */
                i++;
                s = strchr(s, ' ');
                if (s)
                    s++;
        ...
        if (!strcmp((char *)next->name, "range")) { /* propdesc.FORM.Enum */
            ...
            parse_9301_value(
                params, s, type,
                &dpd->FORM.Range.MinimumValue); /* should turn ' ' into \0? */
            s = strchr(s, ' ');
            if (!s)
                continue;
            s++;
            parse_9301_value(
                params, s, type,
                &dpd->FORM.Range.MaximumValue); /* should turn ' ' into \0? */
            s = strchr(s, ' ');
            if (!s)
                continue;
            s++;
            parse_9301_value(
                params, s, type,
                &dpd->FORM.Range.StepSize); /* should turn ' ' into \0? */
...
static int parse_9301_prop_tree(PTPParams *params, xmlNodePtr node,
                                PTPDeviceInfo *di) {
    xmlNodePtr next;
    int cnt;
    unsigned int i;
    ...
    di->DevicePropertiesSupported =
        malloc(cnt * sizeof(di->DevicePropertiesSupported[0]));
    cnt = 0;
    next = xmlFirstElementChild(node);
    while (next) {
        unsigned int p;
        ...
        if (i == params->nrofdeviceproperties) {
            if (!i)
                params->deviceproperties =
                    malloc(sizeof(params->deviceproperties[0]));
            else
                params->deviceproperties =
                    realloc(params->deviceproperties,
                            (i + 1) * sizeof(params->deviceproperties[0]));
            memset(&params->deviceproperties[i], 0,
...
static int parse_9301_event_tree(PTPParams *params, xmlNodePtr node,
                                 PTPDeviceInfo *di) {
    xmlNodePtr next;
    int cnt;
...
static int parse_9301_tree(PTPParams *params, xmlNodePtr node,
                           PTPDeviceInfo *di) {
    xmlNodePtr next;
...
static uint16_t ptp_olympus_parse_output_xml(PTPParams *params, char *data,
                                             int len, xmlNodePtr *code) {
    xmlDocPtr docin;
    ...
    *code = NULL;
    ...
    if (xmlChildElementCount(docroot) != 1) {
        ptp_debug(params, "olympus: x3c: expected 1 child, got %ld",
                  xmlChildElementCount(docroot));
        xmlFreeDoc(docin);
        return PTP_RC_GeneralError;
    }
    ...
    if (strcmp((char *)output->name, "output") != 0) {
        ptp_debug(params,
                  "olympus: x3c node: expected child 'output', but got %s",
                  (char *)output->name);
        xmlFreeDoc(docin);
        return PTP_RC_GeneralError;
    }
...
uint16_t ptp_olympus_opensession(PTPParams *params, unsigned char **data,
                                 unsigned long *len) {
    uint16_t ret;
    PTPContainer ptp;
    PTPDataHandler handler;
...
uint16_t ptp_olympus_getcameraid(PTPParams *params, unsigned char **data,
                                 unsigned long *len) {
    uint16_t ret;
    PTPContainer ptp;
    PTPDataHandler handler;
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_generic_no_data(PTPParams *params, uint16_t code,
    ...
    case PTP_DPFF_Enumeration:
        if (dpd->FORM.Enum.SupportedValue) {
            for (i = 0; i < dpd->FORM.Enum.NumberOfValues; i++)
                ptp_free_devicepropvalue(dpd->DataType,
...
void ptp_free_objectpropdesc(PTPObjectPropDesc *opd) {
    ...
    case PTP_OPFF_Enumeration:
        if (opd->FORM.Enum.SupportedValue) {
            for (i = 0; i < opd->FORM.Enum.NumberOfValues; i++)
                ptp_free_devicepropvalue(opd->DataType,
                                         opd->FORM.Enum.SupportedValue + i);
    ...
    case PTP_OPFF_LongString:
        /* Ignore these presently, we cannot unpack them, so there is nothing to
         * be freed. */
        break;
    default:
 ...
 *					  wich a list of children is desired
 *					  (optional)
 *		numobs			- pointer to uint32_t that takes number of
 *objects
 *
 ...
 * The return value is "0" for all OK, and "1" for capture failed. (not fully
 *confirmed)
 *
 * Return values: Some PTP_RC_* code.
 **/
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getobject_to_handler(PTPParams *params, uint32_t handle,
                                  PTPDataHandler *handler) {
    PTPContainer ptp;
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getpartialobject(PTPParams *params, uint32_t handle,
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_sendobject_from_handler(PTPParams *params, PTPDataHandler *handler,
                                     uint64_t size) {
    PTPContainer ptp;
            ...
            if (ret == PTP_RC_OK) {
                ...
                if ((xmlChildElementCount(code) == 1) &&
                    ...
                    if ((sscanf((char *)code->name, "p%x", &x)) &&
                        (x == propcode)) {
                        ret = parse_9301_propdesc(params,
                                                  xmlFirstElementChild(code),
                                                  devicepropertydesc);
                        xmlFreeDoc(code->doc);
                    }
                }
 ...
 * ptp_ek_getserial:
 * params:	PTPParams*
 *		char**	serial		- contains the serial number of the
 *camera
 ...
 *
 */
uint16_t ptp_ek_getserial(PTPParams *params, unsigned char **data,
                          unsigned int *size) {
    PTPContainer ptp;
 ...
 *
 */
uint16_t ptp_ek_setserial(PTPParams *params, unsigned char *data,
                          unsigned int size) {
    PTPContainer ptp;
...
uint16_t ptp_ek_9007(PTPParams *params, unsigned char **data,
                     unsigned int *size) {
    PTPContainer ptp;
...
uint16_t ptp_ek_900c(PTPParams *params, unsigned char **data,
                     unsigned int *size) {
    PTPContainer ptp;
 ...
 * params:	PTPParams*
 *		PTPDataHandler*	handler	- contains the handler of the
 *object that is to be sent
 *		uint32_t size		- object size
 *
 * Sends object to Responder.
 *
 ...
 *
 */
uint16_t ptp_ek_sendfileobject_from_handler(PTPParams *params,
                                            PTPDataHandler *handler,
                                            uint32_t size) {
    PTPContainer ptp;
 ...
 *
 * Return values: Some PTP_RC_* code.
 * Upon success : PTPObjectHandles        *handles	- filled out with
 handles
 * 		  PTPObjectInfo           **oinfos	- allocated array of
 PTP Object Infos
 * 		  uint32_t                **flags	- allocated array of
 CANON Flags
 *
 **/
uint16_t ptp_canon_get_directory(PTPParams *params, PTPObjectHandles *handles,
    ...
    if (ret != PTP_RC_OK)
        return ret;
    ret = ptp_unpack_canon_directory(params, dir, ptp.Param1, handles, oinfos,
                                     flags);
    free(dir);
    return ret;
}
 ...
 *
 **/
uint16_t ptp_canon_getpairinginfo(PTPParams *params, uint32_t nr,
 ...
 *
 **/
uint16_t ptp_canon_gettreesize(PTPParams *params,
                               PTPCanon_directtransfer_entry **entries,
                               unsigned int *cnt) {
    PTPContainer ptp;
    uint16_t ret;
 ...
 *						  or 0 otherwise
 **/
uint16_t ptp_canon_checkevent(PTPParams *params, PTPContainer *event,
                              int *isevent) {
    uint16_t ret;
    PTPContainer ptp;
...
uint16_t ptp_add_event(PTPParams *params, PTPContainer *evt) {
    if (params->nrofevents)
        params->events = realloc(params->events, sizeof(PTPContainer) *
                                                     (params->nrofevents + 1));
    else
...
uint16_t ptp_check_event(PTPParams *params) {
    ...
    if ((params->deviceinfo.VendorExtensionID == PTP_VENDOR_NIKON) &&
        ...
        if (evtcnt) {
            if (params->nrofevents)
                params->events =
                    realloc(params->events, sizeof(PTPContainer) *
                                                (evtcnt + params->nrofevents));
            else
                params->events = malloc(sizeof(PTPContainer) * evtcnt);
            memcpy(&params->events[params->nrofevents], xevent,
                   evtcnt * sizeof(PTPContainer));
            params->nrofevents += evtcnt;
            free(xevent);
        }
        return PTP_RC_OK;
    }
    /* should not get here ... EOS has no normal PTP events and another queue
     * handling. */
    ...
    if ((params->deviceinfo.VendorExtensionID == PTP_VENDOR_CANON) &&
        ...
        if (isevent)
            goto store_event;
        /* Event Emulate Mode 0 (unset) and 1-5 get interrupt events. 6-7 does
         * not. */
...
store_event:
    if (ret == PTP_RC_OK) {
        ptp_debug(params, "event: nparams=0x%X, code=0x%X, trans_id=0x%X, "
                          "p1=0x%X, p2=0x%X, p3=0x%X",
                  event.Nparam, event.Code, event.Transaction_ID, event.Param1,
...
int ptp_get_one_event(PTPParams *params, PTPContainer *event) {
    ...
    memmove(params->events, params->events + 1,
            sizeof(PTPContainer) * (params->nrofevents - 1));
 ...
 *
 **/
uint16_t ptp_canon_eos_getevent(PTPParams *params,
                                PTPCanon_changes_entry **entries,
                                int *nrofentries) {
    PTPContainer ptp;
    uint16_t ret;
    unsigned int size = 0;
...
uint16_t ptp_check_eos_events(PTPParams *params) {
    ...
    while (1) { /* call it repeatedly until the camera does not report any */
        ...
        if (params->nrofbacklogentries) {
            nentries = realloc(params->backlogentries,
                               sizeof(entries[0]) *
                                   (params->nrofbacklogentries + nrofentries));
            if (!nentries)
                return PTP_RC_GeneralError;
            params->backlogentries = nentries;
            memcpy(nentries + params->nrofbacklogentries, entries,
                   nrofentries * sizeof(entries[0]));
            params->nrofbacklogentries += nrofentries;
            free(entries);
        } else {
...
int ptp_get_one_eos_event(PTPParams *params, PTPCanon_changes_entry *entry) {
    ...
    if (params->nrofbacklogentries > 1) {
        memmove(params->backlogentries, params->backlogentries + 1,
                sizeof(*entry) * (params->nrofbacklogentries - 1));
...
uint16_t ptp_canon_eos_getdevicepropdesc(PTPParams *params, uint16_t propcode,
    ...
    if (dpd->FormFlag == PTP_DPFF_Enumeration) {
        ...
        dpd->FORM.Enum.SupportedValue =
            malloc(sizeof(PTPPropertyValue) * dpd->FORM.Enum.NumberOfValues);
    ...
    if (dpd->DataType == PTP_DTC_STR) {
        dpd->FactoryDefaultValue.str =
            strdup(params->canon_props[i].dpd.FactoryDefaultValue.str);
        dpd->CurrentValue.str =
            strdup(params->canon_props[i].dpd.CurrentValue.str);
    }
...
uint16_t ptp_canon_eos_getobjectinfoex(PTPParams *params, uint32_t storageid,
                                       uint32_t oid, uint32_t unk,
 ...
 *		data		Pointer that receives the malloc()ed memory of the
 *transfer.
 *
 * Return values: Some PTP_RC_* code.
 *
 */
uint16_t ptp_canon_eos_getpartialobject(PTPParams *params, uint32_t oid,
                                        uint32_t offset, uint32_t xsize,
                                        unsigned char **data) {
    PTPContainer ptp;
    unsigned int size = 0;
...
uint16_t ptp_canon_eos_setdevicepropvalueex(PTPParams *params,
                                            unsigned char *data,
                                            unsigned int size) {
    PTPContainer ptp;
...
uint16_t ptp_canon_eos_setdevicepropvalue(PTPParams *params, uint16_t propcode,
    ...
    case PTP_DPC_CANON_EOS_CustomFuncEx:
        ...
        ptp_debug(
            params,
            "ptp2/ptp_canon_eos_setdevicepropvalue: setting EOS prop %x to %s",
            propcode, value->str);
 ...
 *
 **/
uint16_t ptp_canon_getobjectinfo(PTPParams *params, uint32_t store, uint32_t p2,
 ...
 *
 **/
uint16_t ptp_canon_get_objecthandle_by_name(PTPParams *params, char *name,
                                            uint32_t *objectid) {
    uint16_t ret;
    PTPContainer ptp;
    unsigned char *data = NULL;
    ...
    ret = ptp_transaction(params, &ptp, PTP_DP_SENDDATA, (len + 1) * 2 + 1,
                          &data, NULL);
    free(data);
    *objectid = ptp.Param1;
    return ret;
...
uint16_t ptp_nikon_curve_download(PTPParams *params, unsigned char **data,
                                  unsigned int *size) {
    PTPContainer ptp;
    *data = NULL;
    *size = 0;
 ...
 *
 **/
uint16_t ptp_sony_sdioconnect(PTPParams *params, uint32_t p1, uint32_t p2,
                              uint32_t p3) {
    PTPContainer ptp;
    uint16_t ret;
 ...
 *
 **/
uint16_t ptp_sony_get_vendorpropcodes(PTPParams *params, uint16_t **props,
                                      unsigned int *size) {
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *xdata = NULL;
...
uint16_t ptp_generic_getdevicepropdesc(PTPParams *params, uint16_t propcode,
    ...
    if ((params->deviceinfo.VendorExtensionID == PTP_VENDOR_SONY) &&
        ...
        ret = ptp_sony_getdevicepropdesc(params, propcode,
                                         &params->deviceproperties[i].desc);
        if (ret != PTP_RC_OK)
            return ret;
    ...
    if (ptp_operation_issupported(params, PTP_OC_GetDevicePropDesc)) {
        ret = ptp_getdevicepropdesc(params, propcode,
                                    &params->deviceproperties[i].desc);
        if (ret != PTP_RC_OK)
            return ret;
...
uint16_t ptp_generic_setdevicepropvalue(PTPParams *params, uint16_t propcode,
    ...
    if ((params->deviceinfo.VendorExtensionID == PTP_VENDOR_SONY) &&
        ...
        return ptp_sony_setdevicecontrolvaluea(params, propcode, value,
                                               datatype);
 ...
 *
 **/
uint16_t ptp_nikon_get_vendorpropcodes(PTPParams *params, uint16_t **props,
                                       unsigned int *size) {
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *xdata = NULL;
...
uint16_t ptp_nikon_getfileinfoinblock(PTPParams *params, uint32_t p1,
 ...
 *
 **/
uint16_t ptp_nikon_get_liveview_image(PTPParams *params, unsigned char **data,
                                      unsigned int *size) {
    PTPContainer ptp;
 ...
 *
 **/
uint16_t ptp_nikon_get_preview_image(PTPParams *params, unsigned char **xdata,
                                     unsigned int *xsize, uint32_t *handle) {
    PTPContainer ptp;
    uint16_t ret;
 ...
 *
 **/
uint16_t ptp_canon_eos_get_viewfinder_image(PTPParams *params,
                                            unsigned char **data,
                                            unsigned int *size) {
    PTPContainer ptp;
...
uint16_t ptp_canon_eos_get_viewfinder_image_handler(PTPParams *params,
                                                    PTPDataHandler *handler) {
    PTPContainer ptp;
 ...
 *
 **/
uint16_t ptp_nikon_check_event(PTPParams *params, PTPContainer **event,
                               unsigned int *evtcnt) {
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *data = NULL;
 ...
 *
 **/
uint16_t ptp_nikon_getptpipinfo(PTPParams *params, unsigned char **data,
                                unsigned int *size) {
    PTPContainer ptp;
...
uint16_t ptp_nikon_getwifiprofilelist(PTPParams *params) {
    ...
    data = NULL;
    CHECK_PTP_RC(
        ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &data, &size));

    if (size < 2)
        return PTP_RC_Undefined; /* FIXME: Add more precise error code */
    ...
    params->wifi_profiles =
        malloc(params->wifi_profiles_number * sizeof(PTPNIKONWifiProfile));
    memset(params->wifi_profiles, 0,
           params->wifi_profiles_number * sizeof(PTPNIKONWifiProfile));
    ...
    while (profn < params->wifi_profiles_number && pos < size) {
        ...
        if (pos + n + 4 >= size)
            return PTP_RC_Undefined;
        strncpy(params->wifi_profiles[profn].profile_name, (char *)&data[pos],
                n);
        ...
        strncpy(params->wifi_profiles[profn].creation_date, buffer,
                sizeof(params->wifi_profiles[profn].creation_date));
        free(buffer);
        pos += (len * 2 + 1);
        if (pos + 1 >= size)
            return PTP_RC_Undefined;
        ...
        buffer = ptp_unpack_string(params, data, pos, &len);
        strncpy(params->wifi_profiles[profn].lastusage_date, buffer,
                sizeof(params->wifi_profiles[profn].lastusage_date));
        free(buffer);
        pos += (len * 2 + 1);
        if (pos + 5 >= size)
            return PTP_RC_Undefined;
 ...
 *
 **/
uint16_t ptp_nikon_writewifiprofile(PTPParams *params,
    ...
    /**((unsigned int*)&buffer[0x3A]) = profile->ip_address; */ /* Do not
                                                                   reverse bytes
                                                                   */
    buffer[0x3E] = profile->subnet_mask;
    memcpy(&buffer[0x3F], &profile->gateway_address,
           sizeof(profile->gateway_address));
 ...
 *
 **/
uint16_t ptp_mtp_getobjectpropssupported(PTPParams *params, uint16_t ofc,
 ...
 *
 **/
uint16_t ptp_mtp_getobjectpropdesc(PTPParams *params, uint16_t opc,
                                   uint16_t ofc, PTPObjectPropDesc *opd) {
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *data = NULL;
    unsigned int size = 0;
 ...
 *
 **/
uint16_t ptp_mtp_getobjectpropvalue(PTPParams *params, uint32_t oid,
                                    uint16_t opc, PTPPropertyValue *value,
                                    uint16_t datatype) {
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *data = NULL;
    ...
    if (ret == PTP_RC_OK) {
        ...
        if (!ret2) {
            ptp_debug(params,
                      "ptp_mtp_getobjectpropvalue: unpacking DPV failed");
            ret = PTP_RC_GeneralError;
        }
    }
    free(data);
 ...
 *
 **/
uint16_t ptp_mtp_setobjectpropvalue(PTPParams *params, uint32_t oid,
                                    uint16_t opc, PTPPropertyValue *value,
                                    uint16_t datatype) {
    PTPContainer ptp;
    uint16_t ret;
...
uint16_t ptp_mtp_getobjectreferences(PTPParams *params, uint32_t handle,
                                     uint32_t **ohArray, uint32_t *arraylen) {
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *dpv = NULL;
...
uint16_t ptp_mtp_setobjectreferences(PTPParams *params, uint32_t handle,
                                     uint32_t *ohArray, uint32_t arraylen) {
    PTPContainer ptp;
    uint16_t ret;
    uint32_t size;
    ...
    ret = ptp_transaction(params, &ptp, PTP_DP_SENDDATA, size,
                          (unsigned char **)&dpv, NULL);
    free(dpv);
    return ret;
}

uint16_t ptp_mtp_getobjectproplist(PTPParams *params, uint32_t handle,
                                   MTPProperties **props, int *nrofprops) {
    uint16_t ret;
    PTPContainer ptp;
...
uint16_t ptp_mtp_getobjectproplist_single(PTPParams *params, uint32_t handle,
                                          MTPProperties **props,
                                          int *nrofprops) {
    uint16_t ret;
    PTPContainer ptp;
...
uint16_t ptp_mtp_sendobjectproplist(PTPParams *params, uint32_t *store,
                                    uint32_t *parenthandle, uint32_t *handle,
                                    uint16_t objecttype, uint64_t objectsize,
                                    MTPProperties *props, int nrofprops) {
    uint16_t ret;
    PTPContainer ptp;
    unsigned char *opldata = NULL;
    ...
    ret =
        ptp_transaction(params, &ptp, PTP_DP_SENDDATA, oplsize, &opldata, NULL);
    free(opldata);
    *store = ptp.Param1;
    *parenthandle = ptp.Param2;
    *handle = ptp.Param3;
...
uint16_t ptp_mtp_setobjectproplist(PTPParams *params, MTPProperties *props,
                                   int nrofprops) {
    uint16_t ret;
    PTPContainer ptp;
    unsigned char *opldata = NULL;
...
uint16_t ptp_mtpz_sendwmdrmpdapprequest(PTPParams *params,
                                        unsigned char *appcertmsg,
                                        uint32_t size) {
    PTPContainer ptp;

    PTP_CNT_INIT(ptp);
    ptp.Code = PTP_OC_MTP_WMDRMPD_SendWMDRMPDAppRequest;
    return ptp_transaction(params, &ptp, PTP_DP_SENDDATA, size, &appcertmsg,
                           NULL);
}

uint16_t ptp_mtpz_getwmdrmpdappresponse(PTPParams *params,
...
uint16_t ptp_chdk_get_memory(PTPParams *params, int start, int num,
                             unsigned char **buf) {
    PTPContainer ptp;
...
/*
 * Preliminary remote capture over USB code. Corresponding CHDK code is in the
 * ptp-remote-capture-test
 * This is under development and should not be included in builds for general
 * distribution
 */
/*
 * isready: 0: not ready, lowest 2 bits: available image formats, 0x10000000:
 * error
 */
...
uint16_t ptp_chdk_rcgetchunk(PTPParams *params, int fmt,
    ...
    // TODO should allow ptp_getdata_transaction to send chunks directly to
    // file, or to mem
...
uint16_t ptp_chdk_exec_lua(PTPParams *params, char *script, int flags,
                           int *script_id, int *status) {
    uint16_t r;
    PTPContainer ptp;
...
uint16_t ptp_chdk_write_script_msg(PTPParams *params, char *data, unsigned size,
                                   int target_script_id, int *status) {
    uint16_t r;
    PTPContainer ptp;

    // a zero length data phase appears to do bad things, camera stops
    // responding to PTP
    if (!size) {
    ...
    r = ptp_transaction(params, &ptp, PTP_DP_SENDDATA, size,
                        (unsigned char **)&data, NULL);
    if (r != PTP_RC_OK) {
        *status = 0;
        return r;
...
uint16_t ptp_chdk_read_script_msg(PTPParams *params,
...
uint16_t ptp_chdk_get_live_data(PTPParams *params, unsigned flags,
...
uint16_t ptp_chdk_call_function(PTPParams *params, int *args, int size,
                                int *ret) {
    uint16_t r;
    PTPContainer ptp;
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_android_getpartialobject64(PTPParams *params, uint32_t handle,
...
uint16_t ptp_android_sendpartialobject(PTPParams *params, uint32_t handle,
                                       uint64_t offset, unsigned char *object,
                                       uint32_t len) {
    uint32_t err;
    PTPContainer ptp;
...
const char *ptp_get_property_description(PTPParams *params, uint16_t dpc) {
    ...
    struct {
        ...
        {PTP_DPC_CANON_AEBExposureCompensation,
         N_("AEB Exposure Compensation")},
        ...
        {PTP_DPC_CANON_SizeOfOutputDataFromCamera,
         N_("Size of Output Data from Camera")},
        {PTP_DPC_CANON_SizeOfInputDataToCamera,
         N_("Size of Input Data to Camera")},
    ...
    struct {
        ...
        {PTP_DPC_NIKON_BracketingSet,
         N_("NIKON Auto Bracketing Set")},         /* 0xD07C */
        ...
        {PTP_DPC_NIKON_WhiteBalanceBracketStep,
         N_("White Balance Bracket Step")}, /* 0xD0C4 */
        {PTP_DPC_NIKON_WhiteBalanceBracketProgram,
         N_("White Balance Bracket Program")}, /* 0xD0C5 */
    ...
    struct {
        ...
        {PTP_DPC_MTP_SessionInitiatorInfo,
         N_("Session Initiator Info")}, /* D406 */
        {PTP_DPC_MTP_PerceivedDeviceType,
         N_("Perceived Device Type")},                       /* D407 */
...
#define PTP_VAL_BOOL(dpc)                                                      \
    {dpc, 0, N_("Off")}, { dpc, 1, N_("On") }
#define PTP_VENDOR_VAL_BOOL(dpc, vendor)                                       \
    {dpc, vendor, 0, N_("Off")}, { dpc, vendor, 1, N_("On") }
#define PTP_VENDOR_VAL_RBOOL(dpc, vendor)                                      \
    {dpc, vendor, 0, N_("On")}, { dpc, vendor, 1, N_("Off") }
#define PTP_VENDOR_VAL_YN(dpc, vendor)                                         \
    {dpc, vendor, 0, N_("No")}, { dpc, vendor, 1, N_("Yes") }

int ptp_render_property_value(PTPParams *params, uint16_t dpc,
                              PTPDevicePropDesc *dpd, unsigned int length,
                              char *out) {
    unsigned int i;
    int64_t kval;
    ...
    struct {
        ...
        {PTP_DPC_ExposureBiasCompensation, 0, 0.001, 0.0,
         N_("%.1f stops")},                              /* 5010 */
        ...
        {PTP_DPC_NIKON_LightMeter, PTP_VENDOR_NIKON, 0.08333, 0.0,
         N_("%.1f stops")}, /* D10A */
        {PTP_DPC_NIKON_FlashExposureCompensation, PTP_VENDOR_NIKON, 0.16666,
         0.0, N_("%.1f stops")}, /* D126 */
        {PTP_DPC_NIKON_CenterWeightArea, PTP_VENDOR_NIKON, 2.0, 6.0,
         N_("%.0f mm")}, /* D059 */
        {PTP_DPC_NIKON_FocalLengthMin, PTP_VENDOR_NIKON, 0.01, 0.0,
         "%.0f mm"}, /* D0E3 */
        {PTP_DPC_NIKON_FocalLengthMax, PTP_VENDOR_NIKON, 0.01, 0.0,
         "%.0f mm"}, /* D0E4 */
        {PTP_DPC_NIKON_MaxApAtMinFocalLength, PTP_VENDOR_NIKON, 0.01, 0.0,
         "f/%.2g"}, /* D0E5 */
        {PTP_DPC_NIKON_MaxApAtMaxFocalLength, PTP_VENDOR_NIKON, 0.01, 0.0,
         "f/%.2g"}, /* D0E6 */
        {PTP_DPC_NIKON_ExternalFlashCompensation, PTP_VENDOR_NIKON, 1.0 / 6.0,
         0.0, "%.0f"}, /* D124 */
        {PTP_DPC_NIKON_ExposureIndicateStatus, PTP_VENDOR_NIKON, 0.08333, 0.0,
         N_("%.1f stops")}, /* D1B1 - FIXME: check if correct. */
        {PTP_DPC_NIKON_AngleLevel, PTP_VENDOR_NIKON, 1.0 / 65536, 0.0,
         "%.1f'"}, /* 0xD067 */
        {0, 0, 0.0, 0.0, NULL}};
    ...
    struct {
        ...
        {{PTP_DPC_CompressionSetting, 0, 0, N_("JPEG Basic")}, /* 5004 */
         ...
         {PTP_DPC_WhiteBalance, PTP_VENDOR_NIKON, 32786,
         ...
         {PTP_DPC_FlashMode, PTP_VENDOR_NIKON, 32786,
          N_("Rear Curtain Sync + Slow Sync")},
         {PTP_DPC_FlashMode, PTP_VENDOR_NIKON, 32787,
          N_("Red-eye Reduction + Slow Sync")},
         ...
         {PTP_DPC_ExposureProgramMode, PTP_VENDOR_NIKON, 32786,
          N_("Landscape")},
         {PTP_DPC_ExposureProgramMode, PTP_VENDOR_NIKON, 32787, N_("Macro")},
         {PTP_DPC_ExposureProgramMode, PTP_VENDOR_NIKON, 32788, N_("Sports")},
         {PTP_DPC_ExposureProgramMode, PTP_VENDOR_NIKON, 32790,
          N_("Night Landscape")},
         {PTP_DPC_ExposureProgramMode, PTP_VENDOR_NIKON, 32789,
          N_("Night Portrait")},
         ...
         {PTP_DPC_StillCaptureMode, PTP_VENDOR_NIKON, 32784,
          N_("Continuous Low Speed")},
         ...
         {PTP_DPC_StillCaptureMode, PTP_VENDOR_NIKON, 32788,
          N_("Timer + Remote")},
         {PTP_DPC_FocusMeteringMode, 0, 1, N_("Centre-spot")}, /* 501C */
         {PTP_DPC_FocusMeteringMode, 0, 2, N_("Multi-spot")},
         {PTP_DPC_FocusMeteringMode, PTP_VENDOR_NIKON, 32784,
          N_("Single Area")},
         {PTP_DPC_FocusMeteringMode, PTP_VENDOR_NIKON, 32785,
          N_("Closest Subject")},
         {PTP_DPC_FocusMeteringMode, PTP_VENDOR_NIKON, 32786,
          N_("Group Dynamic")},

         /* Nikon specific device properties */
         {PTP_DPC_NIKON_ImageSharpening, PTP_VENDOR_NIKON, 0,
          N_("Auto")}, /* D02A */
         ...
         {PTP_DPC_NIKON_ImageSharpening, PTP_VENDOR_NIKON, 4,
          N_("Medium high")},
         {PTP_DPC_NIKON_ImageSharpening, PTP_VENDOR_NIKON, 5, N_("High")},
         {PTP_DPC_NIKON_ImageSharpening, PTP_VENDOR_NIKON, 6, N_("None")},

         {PTP_DPC_NIKON_ToneCompensation, PTP_VENDOR_NIKON, 0,
          N_("Auto")}, /* D02B */
         {PTP_DPC_NIKON_ToneCompensation, PTP_VENDOR_NIKON, 1, N_("Normal")},
         {PTP_DPC_NIKON_ToneCompensation, PTP_VENDOR_NIKON, 2,
          N_("Low contrast")},
         {PTP_DPC_NIKON_ToneCompensation, PTP_VENDOR_NIKON, 3,
          N_("Medium Low")},
         {PTP_DPC_NIKON_ToneCompensation, PTP_VENDOR_NIKON, 4,
          N_("Medium High")},
         {PTP_DPC_NIKON_ToneCompensation, PTP_VENDOR_NIKON, 5,
          N_("High control")},
         ...
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_AutoDXCrop,
                             PTP_VENDOR_NIKON), /* D033 */

         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_FocusAreaWrap,
                             PTP_VENDOR_NIKON), /* D04F */

         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_EnableCopyright,
                             PTP_VENDOR_NIKON), /* D053 */
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_ISOAuto,
                             PTP_VENDOR_NIKON), /* D054 */

         /* FIXME! this is not ISO Auto (which is a bool) Perhaps ISO Auto
            Time?*/
         ...
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_ExposureCompensation,
                             PTP_VENDOR_NIKON), /*D058 */
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_AELockMode,
                             PTP_VENDOR_NIKON), /* D05E */

         {PTP_DPC_NIKON_AELAFLMode, PTP_VENDOR_NIKON, 0,
          N_("AE/AF Lock")}, /* D05F */
         ...
         {PTP_DPC_NIKON_MeterOff, PTP_VENDOR_NIKON, 0,
          N_("4 seconds")}, /* D062 */
         ...
         {PTP_DPC_NIKON_SelfTimer, PTP_VENDOR_NIKON, 0,
          N_("2 seconds")}, /* D063 */
         ...
         {PTP_DPC_NIKON_MonitorOff, PTP_VENDOR_NIKON, 0,
          N_("10 seconds")}, /* D064 */
         ...
         {PTP_DPC_NIKON_MonitorOff, PTP_VENDOR_NIKON, 5,
          N_("5 seconds")}, /* d80 observed */

         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_ExposureDelayMode,
                             PTP_VENDOR_NIKON), /* D06A */
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_LongExposureNoiseReduction,
                             PTP_VENDOR_NIKON), /* D06B */
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_FileNumberSequence,
                             PTP_VENDOR_NIKON), /* D06C */
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_D7Illumination,
                             PTP_VENDOR_NIKON), /* D06F */

         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_SHSET_CH_GUID_DISP,
                             PTP_VENDOR_NIKON), /* D071 */

         {PTP_DPC_NIKON_FlashShutterSpeed, PTP_VENDOR_NIKON, 0,
          "1/60s"}, /* D075 */
         ...
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_E4ModelingFlash,
                             PTP_VENDOR_NIKON), /* D077 */

         {PTP_DPC_NIKON_BracketSet, PTP_VENDOR_NIKON, 0,
          N_("AE & Flash")}, /* D078 */
         ...
         {PTP_DPC_NIKON_BracketOrder, PTP_VENDOR_NIKON, 0,
          N_("MTR > Under")}, /* D07A */
         {PTP_DPC_NIKON_BracketOrder, PTP_VENDOR_NIKON, 1, N_("Under > MTR")},

         {PTP_DPC_NIKON_F1CenterButtonShootingMode, PTP_VENDOR_NIKON, 0,
          N_("Reset focus point to center")}, /* D080 */
         {PTP_DPC_NIKON_F1CenterButtonShootingMode, PTP_VENDOR_NIKON, 1,
          N_("Highlight active focus point")},
         {PTP_DPC_NIKON_F1CenterButtonShootingMode, PTP_VENDOR_NIKON, 2,
          N_("Unused")},

         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_F3PhotoInfoPlayback,
                             PTP_VENDOR_NIKON), /* D083 */
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_F5CustomizeCommDials,
                             PTP_VENDOR_NIKON), /* D085 */
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_ReverseCommandDial,
                             PTP_VENDOR_NIKON), /* D086 */
         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_F6ButtonsAndDials,
                              PTP_VENDOR_NIKON), /* D089 */
         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_NoCFCard,
                              PTP_VENDOR_NIKON), /* D08A */
         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_AFAreaPoint,
                              PTP_VENDOR_NIKON), /* D08D */

         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_ImageCommentEnable,
                             PTP_VENDOR_NIKON), /* D091 */
         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_ImageRotation,
                              PTP_VENDOR_NIKON), /* D092 */

         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_MovVoice,
                              PTP_VENDOR_NIKON), /* D0A1 */

         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_Bracketing,
                             PTP_VENDOR_NIKON), /* D0C0 */

         /* http://www.rottmerhusen.com/objektives/lensid/nikkor.html is
            complete */
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 0, N_("Unknown")}, /* D0E0 */
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 38,
          "Sigma 70-300mm 1:4-5.6 D APO Macro"},
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 83,
          "AF Nikkor 80-200mm 1:2.8 D ED"},
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 118,
          "AF Nikkor 50mm 1:1.8 D"},
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 127,
          "AF-S Nikkor 18-70mm 1:3.5-4.5G ED DX"},
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 139,
          "AF-S Nikkor 18-200mm 1:3.5-5.6 GED DX VR"},
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 147,
          "AF-S Nikkor 24-70mm 1:2.8G ED DX"},
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 154,
          "AF-S Nikkor 18-55mm 1:3.5-F5.6G DX VR"},
         {PTP_DPC_NIKON_LensID, PTP_VENDOR_NIKON, 159,
          "AF-S Nikkor 35mm 1:1.8G DX"},
         {PTP_DPC_NIKON_FinderISODisp, PTP_VENDOR_NIKON, 0,
          "Show ISO sensitivity"}, /* 0xD0F0 */
         {PTP_DPC_NIKON_FinderISODisp, PTP_VENDOR_NIKON, 1,
          "Show ISO/Easy ISO"},
         {PTP_DPC_NIKON_FinderISODisp, PTP_VENDOR_NIKON, 2, "Show frame count"},

         {PTP_DPC_NIKON_RawCompression, PTP_VENDOR_NIKON, 0,
          N_("Lossless")}, /* D016 */
         {PTP_DPC_NIKON_RawCompression, PTP_VENDOR_NIKON, 1, N_("Lossy")},

         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_ACPower, PTP_VENDOR_NIKON), /* D101 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_AFLockStatus,
                           PTP_VENDOR_NIKON), /* D104 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_AELockStatus,
                           PTP_VENDOR_NIKON), /* D105 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_FVLockStatus,
                           PTP_VENDOR_NIKON), /* D106 */

         {PTP_DPC_NIKON_AutofocusArea, PTP_VENDOR_NIKON, 0,
          N_("Centre")}, /* D108 */
         ...
         {PTP_DPC_NIKON_RecordingMedia, PTP_VENDOR_NIKON, 0,
          N_("Card")}, /* D10B */
         {PTP_DPC_NIKON_RecordingMedia, PTP_VENDOR_NIKON, 1, N_("SDRam")},

         {PTP_DPC_NIKON_USBSpeed, PTP_VENDOR_NIKON, 0,
          N_("USB 1.1")}, /* D10C */
         {PTP_DPC_NIKON_USBSpeed, PTP_VENDOR_NIKON, 1, N_("USB 2.0")},

         {PTP_DPC_NIKON_CameraOrientation, PTP_VENDOR_NIKON, 0,
          "0'"}, /* D10E */
         ...
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_FNumberLock,
                           PTP_VENDOR_NIKON), /* D110 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_ExposureApertureLock,
                           PTP_VENDOR_NIKON), /* D111 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_TVLockSetting,
                           PTP_VENDOR_NIKON), /* D112 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_AVLockSetting,
                           PTP_VENDOR_NIKON), /* D113 */

         {PTP_DPC_NIKON_IllumSetting, PTP_VENDOR_NIKON, 0,
          N_("LCD Backlight")}, /* D114 */
         {PTP_DPC_NIKON_IllumSetting, PTP_VENDOR_NIKON, 1,
          N_("LCD Backlight and Info Display")},

         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_ExternalFlashAttached,
                           PTP_VENDOR_NIKON), /* D120 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_ExternalFlashStatus,
                           PTP_VENDOR_NIKON), /* D121 */

         {PTP_DPC_NIKON_OptimizeImage, PTP_VENDOR_NIKON, 0,
          N_("Normal")}, /* D140 */
         ...
         {PTP_DPC_NIKON_Saturation, PTP_VENDOR_NIKON, 0,
          N_("Normal")}, /* D142 */
         {PTP_DPC_NIKON_Saturation, PTP_VENDOR_NIKON, 1, N_("Moderate")},
         {PTP_DPC_NIKON_Saturation, PTP_VENDOR_NIKON, 2, N_("Enhanced")},

         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_BeepOff,
                              PTP_VENDOR_NIKON), /* D160 */

         {PTP_DPC_NIKON_AutofocusMode, PTP_VENDOR_NIKON, 0,
          N_("AF-S")}, /* D161 */
         ...
         {PTP_DPC_NIKON_AutofocusMode, PTP_VENDOR_NIKON, 4,
          N_("MF (selection)")},

         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_AFAssist,
                              PTP_VENDOR_NIKON), /* D163 */
         ...
         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_ImageReview,
                              PTP_VENDOR_NIKON), /* D165 */

         {PTP_DPC_NIKON_AFAreaIllumination, PTP_VENDOR_NIKON, 0,
          N_("Auto")}, /* D166 */
         ...
         {PTP_DPC_NIKON_FlashCommanderMode, PTP_VENDOR_NIKON, 0,
          N_("TTL")}, /* D168 */
         {PTP_DPC_NIKON_FlashCommanderMode, PTP_VENDOR_NIKON, 1,
          N_("Auto Aperture")},
         {PTP_DPC_NIKON_FlashCommanderMode, PTP_VENDOR_NIKON, 2,
          N_("Full Manual")},

         PTP_VENDOR_VAL_RBOOL(PTP_DPC_NIKON_FlashSign,
                              PTP_VENDOR_NIKON), /* D169 */

         {PTP_DPC_NIKON_RemoteTimeout, PTP_VENDOR_NIKON, 0,
          N_("1 min")}, /* D16B */
         ...
         PTP_VENDOR_VAL_BOOL(PTP_DPC_NIKON_GridDisplay,
                             PTP_VENDOR_NIKON), /* D16C */

         {PTP_DPC_NIKON_FlashModeManualPower, PTP_VENDOR_NIKON, 0,
          N_("Full")}, /* D16D */
         ...
         {PTP_DPC_NIKON_FlashModeCommanderPower, PTP_VENDOR_NIKON, 0,
          N_("Full")}, /* D16E */
         ...
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_WarningDisplay,
                           PTP_VENDOR_NIKON), /* D181 */

         {PTP_DPC_NIKON_BatteryCellKind, PTP_VENDOR_NIKON, 0,
          "LR6 (AA alkaline)"}, /* D182 */
         {PTP_DPC_NIKON_BatteryCellKind, PTP_VENDOR_NIKON, 1, "HR6 (AA Ni-Mh)"},
         {PTP_DPC_NIKON_BatteryCellKind, PTP_VENDOR_NIKON, 2,
          "FR6 (AA Lithium)"},
         ...
         {PTP_DPC_NIKON_InfoDispSetting, PTP_VENDOR_NIKON, 0,
          N_("Auto")}, /* 0xD187 */
         {PTP_DPC_NIKON_InfoDispSetting, PTP_VENDOR_NIKON, 1,
          N_("Dark on light")},
         {PTP_DPC_NIKON_InfoDispSetting, PTP_VENDOR_NIKON, 2,
          N_("Light on dark")},

         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_IndicatorDisp,
                           PTP_VENDOR_NIKON), /* D18D */

         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_LiveViewStatus,
                           PTP_VENDOR_NIKON), /* D1A2 */

         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_ExposureDisplayStatus,
                           PTP_VENDOR_NIKON), /* D1B0 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_InfoDispErrStatus,
                           PTP_VENDOR_NIKON), /* D1B2 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_ExposureIndicateLightup,
                           PTP_VENDOR_NIKON), /* D1B3 */

         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_FlashOpen,
                           PTP_VENDOR_NIKON), /* D1C0 */
         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_FlashCharged,
                           PTP_VENDOR_NIKON), /* D1C1 */

         PTP_VENDOR_VAL_YN(PTP_DPC_NIKON_ManualMovieSetting,
                           PTP_VENDOR_NIKON), /* 0xD0A6 */

         {PTP_DPC_NIKON_FlickerReduction, PTP_VENDOR_NIKON, 0,
          "50Hz"}, /* 0xD034 */
         {PTP_DPC_NIKON_FlickerReduction, PTP_VENDOR_NIKON, 1, "60Hz"},

         {PTP_DPC_NIKON_RemoteMode, PTP_VENDOR_NIKON, 0,
          N_("Delayed Remote")}, /* 0xD035 */
         {PTP_DPC_NIKON_RemoteMode, PTP_VENDOR_NIKON, 1,
          N_("Quick Response")}, /* 0xD035 */
         {PTP_DPC_NIKON_RemoteMode, PTP_VENDOR_NIKON, 2,
          N_("Remote Mirror Up")}, /* 0xD035 */

         {PTP_DPC_NIKON_MonitorOffDelay, PTP_VENDOR_NIKON, 0,
          "5min"}, /* 0xD0b3 */
         {PTP_DPC_NIKON_MonitorOffDelay, PTP_VENDOR_NIKON, 1,
          "10min"}, /* 0xD0b3 */
         {PTP_DPC_NIKON_MonitorOffDelay, PTP_VENDOR_NIKON, 2,
          "15min"}, /* 0xD0b3 */
         {PTP_DPC_NIKON_MonitorOffDelay, PTP_VENDOR_NIKON, 3,
          "20min"}, /* 0xD0b3 */
         {PTP_DPC_NIKON_MonitorOffDelay, PTP_VENDOR_NIKON, 4,
          "30min"}, /* 0xD0b3 */
         ...
         {PTP_DPC_CANON_FullViewFileFormat, PTP_VENDOR_CANON, 0,
          N_("Undefined")},
         ...
         {PTP_DPC_CANON_FlashMode, PTP_VENDOR_CANON, 3,
          N_("Red Eye Suppression")},
         {PTP_DPC_CANON_FlashMode, PTP_VENDOR_CANON, 4,
          N_("Low Speed Synchronization")},
         {PTP_DPC_CANON_FlashMode, PTP_VENDOR_CANON, 5,
          N_("Auto + Red Eye Suppression")},
         {PTP_DPC_CANON_FlashMode, PTP_VENDOR_CANON, 6,
          N_("On + Red Eye Suppression")},
         ...
         {PTP_DPC_CANON_ImageMode, PTP_VENDOR_CANON, 3,
          N_("High-Speed Shutter")},
         {PTP_DPC_CANON_ImageMode, PTP_VENDOR_CANON, 4,
          N_("Low-Speed Shutter")},
         ...
         {PTP_DPC_CANON_DriveMode, PTP_VENDOR_CANON, 0,
          N_("Single-Frame Shooting")},
         {PTP_DPC_CANON_DriveMode, PTP_VENDOR_CANON, 1,
          N_("Continuous Shooting")},
         {PTP_DPC_CANON_DriveMode, PTP_VENDOR_CANON, 2,
          N_("Timer (Single) Shooting")},
         {PTP_DPC_CANON_DriveMode, PTP_VENDOR_CANON, 4,
          N_("Continuous Low-speed Shooting")},
         {PTP_DPC_CANON_DriveMode, PTP_VENDOR_CANON, 5,
          N_("Continuous High-speed Shooting")},
         ...
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 0,
          N_("Center-weighted Metering")},
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 1, N_("Spot Metering")},
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 2,
          N_("Average Metering")},
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 3,
          N_("Evaluative Metering")},
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 4,
          N_("Partial Metering")},
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 5,
          N_("Center-weighted Average Metering")},
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 6,
          N_("Spot Metering Interlocked with AF Frame")},
         {PTP_DPC_CANON_MeteringMode, PTP_VENDOR_CANON, 7,
          N_("Multi-Spot Metering")},
         ...
         {PTP_DPC_CANON_AFDistance, PTP_VENDOR_CANON, 3,
          N_("Zone Focus (Close-up)")},
         {PTP_DPC_CANON_AFDistance, PTP_VENDOR_CANON, 4,
          N_("Zone Focus (Very Close)")},
         {PTP_DPC_CANON_AFDistance, PTP_VENDOR_CANON, 5,
          N_("Zone Focus (Close)")},
         {PTP_DPC_CANON_AFDistance, PTP_VENDOR_CANON, 6,
          N_("Zone Focus (Medium)")},
         {PTP_DPC_CANON_AFDistance, PTP_VENDOR_CANON, 7,
          N_("Zone Focus (Far)")},

         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0, N_("Invalid")},
         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0x1000,
          N_("Focusing Point on Center Only, Manual")},
         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0x1001,
          N_("Focusing Point on Center Only, Auto")},
         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0x3000,
          N_("Multiple Focusing Points (No Specification), Manual")},
         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0x3001,
          N_("Multiple Focusing Points, Auto")},
         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0x3002,
          N_("Multiple Focusing Points (Right)")},
         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0x3003,
          N_("Multiple Focusing Points (Center)")},
         {PTP_DPC_CANON_FocusingPoint, PTP_VENDOR_CANON, 0x3004,
          N_("Multiple Focusing Points (Left)")},
         ...
         {PTP_DPC_CANON_WhiteBalance, PTP_VENDOR_CANON, 9,
          N_("Color Temperature")},
         {PTP_DPC_CANON_WhiteBalance, PTP_VENDOR_CANON, 10,
          N_("Custom Whitebalance PC-1")},
         {PTP_DPC_CANON_WhiteBalance, PTP_VENDOR_CANON, 11,
          N_("Custom Whitebalance PC-2")},
         {PTP_DPC_CANON_WhiteBalance, PTP_VENDOR_CANON, 12,
          N_("Custom Whitebalance PC-3")},
         {PTP_DPC_CANON_WhiteBalance, PTP_VENDOR_CANON, 13,
          N_("Missing Number")},
         {PTP_DPC_CANON_WhiteBalance, PTP_VENDOR_CANON, 14,
          N_("Fluorescent H")}, /* dup? */

         {PTP_DPC_CANON_SlowShutterSetting, PTP_VENDOR_CANON, 0, N_("Off")},
         {PTP_DPC_CANON_SlowShutterSetting, PTP_VENDOR_CANON, 1,
          N_("Night View")},
         {PTP_DPC_CANON_SlowShutterSetting, PTP_VENDOR_CANON, 2, N_("On")},
         {PTP_DPC_CANON_SlowShutterSetting, PTP_VENDOR_CANON, 3,
          N_("Low-speed shutter function not available")},
         ...
         {PTP_DPC_CANON_ParameterSet, PTP_VENDOR_CANON, 0x08,
          N_("Standard Development Parameters")},
         {PTP_DPC_CANON_ParameterSet, PTP_VENDOR_CANON, 0x10,
          N_("Development Parameters 1")},
         {PTP_DPC_CANON_ParameterSet, PTP_VENDOR_CANON, 0x20,
          N_("Development Parameters 2")},
         {PTP_DPC_CANON_ParameterSet, PTP_VENDOR_CANON, 0x40,
          N_("Development Parameters 3")},
    ...
    for (i = 0; ptp_value_trans[i].dpc != 0; i++) {
        if ((ptp_value_trans[i].dpc == dpc) &&
            (((ptp_value_trans[i].dpc & 0xf000) == 0x5000) ||
             (ptp_value_trans[i].vendor ==
              params->deviceinfo.VendorExtensionID))) {
            double value = _value_to_num(&(dpd->CurrentValue), dpd->DataType);

            return snprintf(out, length, _(ptp_value_trans[i].format),
...
struct {
    ...
    {PTP_OFC_MTP_MSExcelSpreadsheetXLS,
     N_("Microsoft Excel Spreadsheet (.xls)")},
    {PTP_OFC_MTP_MSPowerpointPresentationPPT,
     N_("Microsoft Powerpoint (.ppt)")},
    ...
    {PTP_OFC_MTP_UndefinedWindowsExecutable,
     N_("Undefined Windows Executable")},
    ...
    if (!(ofc & 0x8000)) {
        for (i = 0; i < sizeof(ptp_ofc_trans) / sizeof(ptp_ofc_trans[0]); i++)
            if (ofc == ptp_ofc_trans[i].ofc)
                return snprintf(txt, spaceleft, "%s",
                                _(ptp_ofc_trans[i].format));
    } else {
        ...
        case PTP_VENDOR_MTP:
            for (i = 0;
                 i < sizeof(ptp_ofc_mtp_trans) / sizeof(ptp_ofc_mtp_trans[0]);
                 i++)
                if (ofc == ptp_ofc_mtp_trans[i].ofc)
                    return snprintf(txt, spaceleft, "%s",
                                    _(ptp_ofc_mtp_trans[i].format));
            break;
        default:
            break;
        }
...
struct {
    ...
    {PTP_OC_MTP_GetInterdependendPropdesc,
     N_("Get interdependent property description")},
    ...
    /* WMDRMPD Extensions */
    {PTP_OC_MTP_WMDRMPD_GetSecureTimeChallenge,
     N_("Get secure time challenge")},
    ...
    {PTP_OC_MTP_WMDRMPD_SendMeterChallengeQuery,
     N_("Send meter challenge query")},
    ...
    /* WMPPD Extensions */
    {PTP_OC_MTP_WMPPD_ReportAddedDeletedItems,
     N_("Report Added/Deleted Items")},
    {PTP_OC_MTP_WMPPD_ReportAcquiredItems, N_("Report Acquired Items")},
    {PTP_OC_MTP_WMPPD_PlaylistObjectPref,
     N_("Get transferable playlist types")},

    /* WMDRMPD Extensions... these have no identifiers associated with them */
    {PTP_OC_MTP_WMDRMPD_SendWMDRMPDAppRequest,
     N_("Send WMDRM-PD Application Request")},
    {PTP_OC_MTP_WMDRMPD_GetWMDRMPDAppResponse,
     N_("Get WMDRM-PD Application Response")},
    {PTP_OC_MTP_WMDRMPD_EnableTrustedFilesOperations,
     N_("Enable trusted file operations")},
    {PTP_OC_MTP_WMDRMPD_DisableTrustedFilesOperations,
     N_("Disable trusted file operations")},
    {PTP_OC_MTP_WMDRMPD_EndTrustedAppSession,
     N_("End trusted application session")},
    ...
    /* WMDRMND Extensions */
    {PTP_OC_MTP_WMDRMND_SendRegistrationRequest,
     N_("Send Registration Request")},
    {PTP_OC_MTP_WMDRMND_GetRegistrationResponse,
     N_("Get Registration Response")},
    ...
    {PTP_OC_MTP_WMDRMND_SendWMDRMNDLicenseRequest,
     N_("Send WMDRM-ND License Request")},
    {PTP_OC_MTP_WMDRMND_GetWMDRMNDLicenseResponse,
     N_("Get WMDRM-ND License Response")},
    ...
    {PTP_OC_ANDROID_GetPartialObject64,
     N_("Get Partial Object (64bit Offset)")},
...
int ptp_render_opcode(PTPParams *params, uint16_t opcode, int spaceleft,
                      char *txt) {
    unsigned int i;

    if (!(opcode & 0x8000)) {
        for (i = 0; i < sizeof(ptp_opcode_trans) / sizeof(ptp_opcode_trans[0]);
             i++)
            if (opcode == ptp_opcode_trans[i].opcode)
                return snprintf(txt, spaceleft, "%s",
                                _(ptp_opcode_trans[i].name));
    } else {
        switch (params->deviceinfo.VendorExtensionID) {
        case PTP_VENDOR_MICROSOFT:
        case PTP_VENDOR_MTP:
            for (i = 0; i < sizeof(ptp_opcode_mtp_trans) /
                                sizeof(ptp_opcode_mtp_trans[0]);
                 i++)
                if (opcode == ptp_opcode_mtp_trans[i].opcode)
                    return snprintf(txt, spaceleft, "%s",
                                    _(ptp_opcode_mtp_trans[i].name));
            break;
        default:
            break;
        }
 ...
 */
MTPProperties *ptp_get_new_object_prop_entry(MTPProperties **props,
                                             int *nrofprops) {
...
void ptp_destroy_object_prop(MTPProperties *prop) {
    ...
    else if ((prop->datatype == PTP_DTC_AINT8 ||
              prop->datatype == PTP_DTC_AINT16 ||
              prop->datatype == PTP_DTC_AINT32 ||
              prop->datatype == PTP_DTC_AINT64 ||
              prop->datatype == PTP_DTC_AINT128 ||
              prop->datatype == PTP_DTC_AUINT8 ||
              prop->datatype == PTP_DTC_AUINT16 ||
              prop->datatype == PTP_DTC_AUINT32 ||
              prop->datatype == PTP_DTC_AUINT64 ||
              prop->datatype == PTP_DTC_AUINT128) &&
 ...
 */
MTPProperties *ptp_find_object_prop_in_cache(PTPParams *params,
...
void ptp_remove_object_from_cache(PTPParams *params, uint32_t handle) {
    ...
    params->objects =
        realloc(params->objects, sizeof(PTPObject) * params->nrofobjects);
}
...
uint16_t ptp_object_find(PTPParams *params, uint32_t handle,
                         PTPObject **retob) {
    PTPObject tmpob;

    tmpob.oid = handle;
    *retob = bsearch(&tmpob, params->objects, params->nrofobjects,
                     sizeof(tmpob), _cmp_ob);
    if (!*retob)
        return PTP_RC_GeneralError;
    return PTP_RC_OK;
}

/* Binary search in objects + insert of not found. Needs "objects" to be a
 * sorted by objectid list!  */
uint16_t ptp_object_find_or_insert(PTPParams *params, uint32_t handle,
                                   PTPObject **retob) {
    ...
    *retob = NULL;
    ...
    /*ptp_debug (params, "searching %08x, total=%d", handle,
     * params->nrofobjects);*/
    while (1) {
        cursor = (end - begin) / 2 + begin;
        /*ptp_debug (params, "ob %d: %08x [%d-%d]", cursor,
         * params->objects[cursor].oid, begin, end);*/
    ...
    if ((begin == 0) && (handle < params->objects[0].oid)) {
        ...
        if ((end == params->nrofobjects - 1) &&
            (handle > params->objects[end].oid))
            insertat = end + 1;
        else
            insertat = begin + 1;
    }
    /*ptp_debug (params, "inserting oid %x at [%x,%x], begin=%d, end=%d,
     * insertat=%d\n", handle, params->objects[begin].oid,
     * params->objects[end].oid, begin, end, insertat);*/
    newobs =
        realloc(params->objects, sizeof(PTPObject) * (params->nrofobjects + 1));
    if (!newobs)
        return PTP_RC_GeneralError;
...
uint16_t ptp_object_want(PTPParams *params, uint32_t handle, unsigned int want,
                         PTPObject **retob) {
    uint16_t ret;
    PTPObject *ob;
...
#define X                                                                      \
    (PTPOBJECT_OBJECTINFO_LOADED | PTPOBJECT_STORAGEID_LOADED |                \
     PTPOBJECT_PARENTOBJECT_LOADED)
    if ((want & X) && ((ob->flags & X) != X)) {
        uint32_t saveparent = 0;

        /* One EOS issue, where getobjecthandles(root) returns obs without root
         * flag. */
        ...
        if ((params->deviceinfo.VendorExtensionID == PTP_VENDOR_CANON) &&
            ...
            ret = ptp_canon_getobjectinfo(params, ob->oi.StorageID, 0,
                                          ob->oi.ParentObject, handle, &ents,
                                          &numents);
...
#undef X
    if ((want & PTPOBJECT_MTPPROPLIST_LOADED) &&
        ...
        ret = ptp_mtp_getobjectproplist_single(params, handle, &props,
                                               &nrofprops);
        if (ret != PTP_RC_OK)
            goto fallback;
        ob->mtpprops = props;
        ...
        if (params->device_flags & DEVICE_FLAG_PROPLIST_OVERRIDES_OI) {
            ...
            for (i = 0; i < ob->nrofmtpprops; i++, prop++) {
                ...
                case PTP_OPC_ObjectSize:
                    if (prop->datatype == PTP_DTC_UINT64) {
                        if (prop->propval.u64 > 0xFFFFFFFFU)
                            ...
                            ob->oi.ObjectCompressedSize =
                                (uint32_t)prop->propval.u64;
                ...
                case PTP_OPC_DateModified:
                    ob->oi.ModificationDate =
                        ptp_unpack_PTPTIME(prop->propval.str);
                    break;
    ...
    ptp_debug(params,
              ...
              handle, want, ob->flags);
    return PTP_RC_GeneralError;
}

uint16_t ptp_add_object_to_cache(PTPParams *params, uint32_t handle) {
    PTPObject *ob;
    return ptp_object_want(params, handle, PTPOBJECT_OBJECTINFO_LOADED |
                                               PTPOBJECT_MTPPROPLIST_LOADED,
                           &ob);
}
