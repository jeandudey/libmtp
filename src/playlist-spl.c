...
static void write_from_spl_text_t(LIBMTP_mtpdevice_t *device, const int fd,
                                  text_t *p);
static void free_spl_text_t(text_t *p);
static void print_spl_text_t(text_t *p);
static uint32_t trackno_spl_text_t(text_t *p);
static void tracks_from_spl_text_t(text_t *p, uint32_t *tracks,
                                   LIBMTP_folder_t *folders,
                                   LIBMTP_file_t *files);
static void spl_text_t_from_tracks(text_t **p, uint32_t *tracks,
                                   const uint32_t trackno,
                                   const uint32_t ver_major,
                                   const uint32_t ver_minor, char *dnse,
                                   LIBMTP_folder_t *folders,
                                   LIBMTP_file_t *files);

static uint32_t discover_id_from_filepath(
    const char *s, LIBMTP_folder_t *folders,
    LIBMTP_file_t *files); // TODO add file/dir cached args
static void discover_filepath_from_id(char **p, uint32_t track,
                                      LIBMTP_folder_t *folders,
                                      LIBMTP_file_t *files);
static void find_folder_name(LIBMTP_folder_t *folders, uint32_t *id,
                             char **name);
static uint32_t find_folder_id(LIBMTP_folder_t *folders, uint32_t parent,
                               char *name);
...
void spl_to_playlist_t(LIBMTP_mtpdevice_t *device, PTPObjectInfo *oi,
    ...
    if (unlink(tmpname) < 0)
        LIBMTP_ERROR("failed to delete temp file for %s.spl -> %s, errno=%s\n",
                     pl->name, tmpname, strerror(errno));
    int ret = LIBMTP_Get_File_To_File_Descriptor(device, pl->playlist_id, fd,
                                                 NULL, NULL);
    if (ret < 0) {
        // FIXME     add_ptp_error_to_errorstack(device, ret,
        // "LIBMTP_Get_Playlist: Could not get .spl playlist file.");
        close(fd);
...
int playlist_t_to_spl(LIBMTP_mtpdevice_t *device, LIBMTP_playlist_t *const pl) {
    ...
    char tmpname[] =
        "/tmp/mtp-spl2pl-XXXXXX"; // must be a var since mkstemp modifies it
    ...
    if (FLAG_PLAYLIST_SPL_V2(ptp_usb))
        ...
        ver_major = 1; // FLAG_PLAYLIST_SPL_V1()
    ...
    spl_text_t_from_tracks(&t, pl->tracks, pl->no_tracks, ver_major, ver_minor,
                           NULL, folders, files);
    ...
    f->filesize =
        lseek(fd, 0, SEEK_CUR); // file desc is currently at end of file
 ...
 * @return text_t* a linked list of lines of text, id is left blank, NULL if
 * nothing read in
 */
static text_t *read_into_spl_text_t(LIBMTP_mtpdevice_t *device, const int fd) {
    // set MAXREAD to match STRING_BUFFER_LENGTH in unicode.h conversion
    // function
    ...
    while (!eof) {
        ...
        if (rdcnt < 0)
            ...
            if (it - t == MAXREAD)
                LIBMTP_ERROR("error -- buffer too small to read in .spl "
                             "playlist entry\n");
        ...
        while (it < it_end) {
            ...
            if ((iw - w) >= 2 && // we must have at least two bytes
                *(iw - 1) == '\0' &&
                *(iw - 2) == '\0' && // 0x0000 is end-of-string
            ...
            if (iw >= w + WSIZE) {
                ...
                //   proceeding on as if everything is okay, probably losing a
                //   track
                //   from the playlist
                LIBMTP_ERROR("ERROR %s:%u:%s(): buffer overflow! .spl line too "
                             "long @ %zuB\n",
        ...
        // otherwise we need to save our partial work, if we're not quiting
        // (eof).
        // there is nothing special we need to do, to achieve this since the
        // partially completed string will sit in 'w' until we return to
        // complete
        // the line
    }
...
static void write_from_spl_text_t(LIBMTP_mtpdevice_t *device, const int fd,
    ...
    while (p != NULL) {
        ...
        if (ret < 0)
            ...
            LIBMTP_ERROR(
                "write spl file wrong number of bytes ret=%d len=%d '%s'\n",
...
// returns p = NULL on failure, else the filepath to the track including track
// name, allocated as a correct length string
static void discover_filepath_from_id(char **p, uint32_t track,
                                      LIBMTP_folder_t *folders,
                                      LIBMTP_file_t *files) {
    // fill in a string from the right side since we don't know the root till
    // the end
    const int M = 1024;
    char w[M];
    ...
    *p = NULL;
    ...
    while (id != 0) {
        ...
        if (f == NULL)
            return; // fail if the next part of the path couldn't be found
        iw = iw - (strlen(f) + 1);
        // FIXME: check for string overflow before it occurs
        strcpy(iw, f);
        iw[strlen(f)] = '\\';
 ...
 */
static uint32_t discover_id_from_filepath(const char *s,
                                          LIBMTP_folder_t *folders,
                                          LIBMTP_file_t *files) {
