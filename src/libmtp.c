 ...
 */
static int register_filetype(char const *const description,
                             LIBMTP_filetype_t const id, uint16_t const ptp_id);
static void init_filemap();
static int register_property(char const *const description,
...
static uint16_t
map_libmtp_property_to_ptp_property(LIBMTP_property_t inproperty);
...
static char *get_string_from_object(LIBMTP_mtpdevice_t *device,
                                    uint32_t const object_id,
                                    uint16_t const attribute_id);
static uint64_t get_u64_from_object(LIBMTP_mtpdevice_t *device,
                                    uint32_t const object_id,
                                    uint16_t const attribute_id,
                                    uint64_t const value_default);
static uint32_t get_u32_from_object(LIBMTP_mtpdevice_t *device,
                                    uint32_t const object_id,
                                    uint16_t const attribute_id,
                                    uint32_t const value_default);
static uint16_t get_u16_from_object(LIBMTP_mtpdevice_t *device,
                                    uint32_t const object_id,
                                    uint16_t const attribute_id,
                                    uint16_t const value_default);
static uint8_t get_u8_from_object(LIBMTP_mtpdevice_t *device,
                                  uint32_t const object_id,
                                  uint16_t const attribute_id,
                                  uint8_t const value_default);
static int set_object_string(LIBMTP_mtpdevice_t *device,
                             uint32_t const object_id,
                             uint16_t const attribute_id,
...
static void get_track_metadata(LIBMTP_mtpdevice_t *device,
                               uint16_t objectformat, LIBMTP_track_t *track);
static LIBMTP_folder_t *get_subfolders_for_folder(LIBMTP_folder_t *list,
                                                  uint32_t parent);
static int
create_new_abstract_list(LIBMTP_mtpdevice_t *device, char const *const name,
                         char const *const artist, char const *const composer,
                         char const *const genre, uint32_t const parenthandle,
                         uint32_t const storageid, uint16_t const objectformat,
                         char const *const suffix, uint32_t *const newid,
                         uint32_t const *const tracks,
                         uint32_t const no_tracks);
static int
update_abstract_list(LIBMTP_mtpdevice_t *device, char const *const name,
                     char const *const artist, char const *const composer,
                     char const *const genre, uint32_t const objecthandle,
                     uint16_t const objectformat, uint32_t const *const tracks,
                     uint32_t const no_tracks);
static int send_file_object_info(LIBMTP_mtpdevice_t *device,
                                 LIBMTP_file_t *filedata);
static void add_object_to_cache(LIBMTP_mtpdevice_t *device, uint32_t object_id);
static void update_metadata_cache(LIBMTP_mtpdevice_t *device,
                                  uint32_t object_id);
static int set_object_filename(LIBMTP_mtpdevice_t *device, uint32_t object_id,
                               uint16_t ptp_type, const char **newname);
static char *generate_unique_filename(PTPParams *params,
                                      char const *const filename);
static int check_filename_exists(PTPParams *params, char const *const filename);

/**
 * These are to wrap the get/put handlers to convert from the MTP types to PTP
 * types
 * in a reliable way
 */
...
static uint16_t get_func_wrapper(PTPParams *params, void *priv,
                                 unsigned long wantlen, unsigned char *data,
                                 unsigned long *gotlen);
static uint16_t put_func_wrapper(PTPParams *params, void *priv,
...
static void init_propertymap() {
    ...
    register_property("Birthdate", LIBMTP_PROPERTY_Birthdate,
                      PTP_OPC_Birthdate);
    ...
    register_property("Webmaster", LIBMTP_PROPERTY_Webmaster,
                      PTP_OPC_Webmaster);
...
/**
 * Returns the PTP property that maps to a certain libmtp internal property
 * type.
 ...
 */
static LIBMTP_property_t
map_ptp_property_to_libmtp_property(uint16_t inproperty) {
    propertymap_t *current;
...
void LIBMTP_Init(void) {
    ...
    if (env_debug) {
        ...
        if (debug_flags != LONG_MIN && debug_flags != LONG_MAX &&
            ...
            fprintf(stderr,
                    "LIBMTP_Init: error setting debug flags from environment "
                    "value \"%s\"\n",
                    env_debug);
        }
    }
...
int LIBMTP_Get_Allowed_Property_Values(LIBMTP_mtpdevice_t *device,
    ...
    ret = ptp_mtp_getobjectpropdesc(
        device->params, map_libmtp_property_to_ptp_property(property),
        map_libmtp_type_to_ptp_type(filetype), &opd);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(device, ret, "LIBMTP_Get_Allowed_Property_"
                                                 "Values(): could not get "
    ...
    if (opd.FormFlag == PTP_OPFF_Enumeration) {
        ...
        case PTP_DTC_INT8:
            allowed_vals->i8vals =
                malloc(sizeof(int8_t) * opd.FORM.Enum.NumberOfValues);
            allowed_vals->datatype = LIBMTP_DATATYPE_INT8;
            break;
        case PTP_DTC_UINT8:
            allowed_vals->u8vals =
                malloc(sizeof(uint8_t) * opd.FORM.Enum.NumberOfValues);
            allowed_vals->datatype = LIBMTP_DATATYPE_UINT8;
            break;
        case PTP_DTC_INT16:
            allowed_vals->i16vals =
                malloc(sizeof(int16_t) * opd.FORM.Enum.NumberOfValues);
            allowed_vals->datatype = LIBMTP_DATATYPE_INT16;
            break;
        case PTP_DTC_UINT16:
            allowed_vals->u16vals =
                malloc(sizeof(uint16_t) * opd.FORM.Enum.NumberOfValues);
            allowed_vals->datatype = LIBMTP_DATATYPE_UINT16;
            break;
        case PTP_DTC_INT32:
            allowed_vals->i32vals =
                malloc(sizeof(int32_t) * opd.FORM.Enum.NumberOfValues);
            allowed_vals->datatype = LIBMTP_DATATYPE_INT32;
            break;
        case PTP_DTC_UINT32:
            allowed_vals->u32vals =
                malloc(sizeof(uint32_t) * opd.FORM.Enum.NumberOfValues);
            allowed_vals->datatype = LIBMTP_DATATYPE_UINT32;
            break;
        case PTP_DTC_INT64:
            allowed_vals->i64vals =
                malloc(sizeof(int64_t) * opd.FORM.Enum.NumberOfValues);
            allowed_vals->datatype = LIBMTP_DATATYPE_INT64;
            break;
        case PTP_DTC_UINT64:
            allowed_vals->u64vals =
                malloc(sizeof(uint64_t) * opd.FORM.Enum.NumberOfValues);
 ...
 */
int LIBMTP_Is_Property_Supported(LIBMTP_mtpdevice_t *device,
                                 LIBMTP_property_t const property,
                                 LIBMTP_filetype_t const filetype) {
    uint16_t *props = NULL;
    uint32_t propcnt = 0;
    ...
    if (!ptp_operation_issupported(device->params,
                                   PTP_OC_MTP_GetObjectPropsSupported))
        return 0;

    ret = ptp_mtp_getobjectpropssupported(device->params,
                                          map_libmtp_type_to_ptp_type(filetype),
                                          &propcnt, &props);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(device, ret, "LIBMTP_Is_Property_Supported("
 ...
 * @return the value
 */
uint64_t LIBMTP_Get_u64_From_Object(LIBMTP_mtpdevice_t *device,
                                    uint32_t const object_id,
                                    LIBMTP_property_t const attribute_id,
                                    uint64_t const value_default) {
    return get_u64_from_object(
        device, object_id, map_libmtp_property_to_ptp_property(attribute_id),
        value_default);
}
 ...
 * @return a value
 */
uint16_t LIBMTP_Get_u16_From_Object(LIBMTP_mtpdevice_t *device,
                                    uint32_t const object_id,
                                    LIBMTP_property_t const attribute_id,
                                    uint16_t const value_default) {
    return get_u16_from_object(
        device, object_id, map_libmtp_property_to_ptp_property(attribute_id),
        value_default);
}
 ...
 * @return a value
 */
uint8_t LIBMTP_Get_u8_From_Object(LIBMTP_mtpdevice_t *device,
                                  uint32_t const object_id,
                                  LIBMTP_property_t const attribute_id,
                                  uint8_t const value_default) {
...
int LIBMTP_Set_Object_u8(LIBMTP_mtpdevice_t *device, uint32_t const object_id,
                         LIBMTP_property_t const attribute_id,
                         uint8_t const value) {
    return set_object_u8(device, object_id,
                         map_libmtp_property_to_ptp_property(attribute_id),
                         value);
}
...
static uint64_t get_u64_from_object(LIBMTP_mtpdevice_t *device,
    ...
    if (ret == PTP_RC_OK) {
        ...
        add_ptp_error_to_errorstack(device, ret, "get_u64_from_object(): could "
                                                 "not get unsigned 64bit "
                                                 "integer from object.");
    }
...
static uint32_t get_u32_from_object(LIBMTP_mtpdevice_t *device,
    ...
    if (ret == PTP_RC_OK) {
        ...
        add_ptp_error_to_errorstack(device, ret, "get_u32_from_object(): could "
                                                 "not get unsigned 32bit "
                                                 "integer from object.");
    }
    return retval;
}
...
static uint16_t get_u16_from_object(LIBMTP_mtpdevice_t *device,
    ...
    if (ret == PTP_RC_OK) {
        ...
        add_ptp_error_to_errorstack(device, ret, "get_u16_from_object(): could "
                                                 "not get unsigned 16bit "
                                                 "integer from object.");
    }
...
static uint8_t get_u8_from_object(LIBMTP_mtpdevice_t *device,
    ...
    if (ret == PTP_RC_OK) {
        ...
        add_ptp_error_to_errorstack(device, ret, "get_u8_from_object(): could "
                                                 "not get unsigned 8bit "
                                                 "integer from object.");
    }
...
static int set_object_string(LIBMTP_mtpdevice_t *device,
    ...
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(
            device, ret, "set_object_string(): could not set object string.");
        return -1;
    }
            ...
            if (element) {
                ...
                if (i < strlen(element)) {
                    ...
                    if (i > majstart && i < strlen(element)) {
                        ...
                        int minor = 0;
                        char *majorstr =
                            strndup(element + majstart, i - majstart);
                        char *minorstr =
                            strndup(element + i + 1, strlen(element) - i - 1);
                        major = atoi(majorstr);
                        minor = atoi(minorstr);
                        free(majorstr);
                        free(minorstr);
                        ...
                        if (mtpdevice->extensions == NULL) {
                            ...
                            LIBMTP_device_extension_t *tmp =
                                mtpdevice->extensions;
                            while (tmp->next != NULL)
                                tmp = tmp->next;
                            tmp->next = extension;
                        }
                        // printf("    Major: \"%s\" (parsed %d) Minor: \"%s\"
                        // (parsed %d)\n",
 ...
 */
LIBMTP_mtpdevice_t *
LIBMTP_Open_Raw_Device_Uncached(LIBMTP_raw_device_t *rawdevice) {
    ...
    if (mtp_device == NULL) {
        /* There has been an memory allocation error. We are going to ignore
           this
           device and attempt to continue */

        /* TODO: This error statement could probably be a bit more robust */
        LIBMTP_ERROR(
            "LIBMTP PANIC: connect_usb_devices encountered a memory "
    ...
    if (ptp_getdeviceinfo(current_params, &current_params->deviceinfo) !=
        PTP_RC_OK) {
        LIBMTP_ERROR(
            "LIBMTP PANIC: Unable to read device information on device "
     ...
     */
    {
        ...
        if (is_microsoft_com_wpdna && is_sonyericsson_com_se && !is_android) {
            /*
             * The Aricent stack seems to be detected by providing WPDNA, the
             * SonyEricsson
             * extension and NO Android extension.
             */
            ptp_usb->rawdevice.device_entry.device_flags |=
                DEVICE_FLAGS_ARICENT_BUGS;
            LIBMTP_INFO("Aricent MTP stack device detected, assigning default "
                        "bug flags\n");
        } else if (is_android) {
            /*
             * If bugs are fixed in later versions, test on tmpext->major,
             * tmpext->minor
             */
            ptp_usb->rawdevice.device_entry.device_flags |=
    ...
    if (FLAG_FLAC_IS_UNKNOWN(ptp_usb)) {
        for (i = 0; i < current_params->deviceinfo.ImageFormats_len; i++) {
            if (current_params->deviceinfo.ImageFormats[i] ==
                PTP_OFC_MTP_FLAC) {
                /* This is not unknown anymore, unflag it */
                ptp_usb->rawdevice.device_entry.device_flags &=
    ...
    if (ptp_operation_issupported(current_params,
                                  PTP_OC_MTP_GetObjectPropsSupported)) {
        for (i = 0; i < current_params->deviceinfo.ImageFormats_len; i++) {
            PTPObjectPropDesc opd;

            if (ptp_mtp_getobjectpropdesc(
                    current_params, PTP_OPC_ObjectSize,
                ...
                if (opd.DataType == PTP_DTC_UINT32) {
                    if (bs == 0) {
                        ...
                        LIBMTP_ERROR("LIBMTP PANIC: "
                                     "different objects support different "
                                     "object sizes!\n");
                        bs = 0;
                        break;
                    }
    ...
    if (!FLAG_BROKEN_BATTERY_LEVEL(ptp_usb) &&
        ...
        if (ptp_getdevicepropdesc(current_params, PTP_DPC_BatteryLevel, &dpd) !=
            PTP_RC_OK) {
            add_error_to_errorstack(
                mtp_device, LIBMTP_ERROR_CONNECTING,
    ...
    if (LIBMTP_Get_Storage(mtp_device, LIBMTP_STORAGE_SORTBY_NOTSORTED) == -1) {
        add_error_to_errorstack(mtp_device, LIBMTP_ERROR_GENERAL,
        ...
        while (tmpext != NULL) {
            if (!strcmp(tmpext->name, "microsoft.com/MTPZ")) {
                ...
                if (PTP_RC_OK == ptp_mtpz_handshake(mtp_device->params)) {
                    ...
                    LIBMTP_INFO("(MTPZ) Failure - could not authenticate with "
                                "device.\n");
                }
                break;
            }
 ...
 * @param event contains a pointer to be filled in with the event retrieved if
 * the call
 * is successful.
...
int LIBMTP_Read_Event(LIBMTP_mtpdevice_t *device, LIBMTP_event_t *event,
    ...
    *event = LIBMTP_EVENT_NONE;
    ...
    case PTP_EC_RequestObjectTransfer:
        LIBMTP_INFO(
            "Received event PTP_EC_RequestObjectTransfer in session %u\n",
            session_id);
        break;
...
static void add_ptp_error_to_errorstack(LIBMTP_mtpdevice_t *device,
    ...
    if (device == NULL) {
        ...
        snprintf(outstr, sizeof(outstr), "Error %04x: %s", ptp_error,
                 ptp_strerror(ptp_error));
        outstr[sizeof(outstr) - 1] = '\0';
        add_error_to_errorstack(device, LIBMTP_ERROR_PTP_LAYER, outstr);
    }
}
...
void LIBMTP_Clear_Errorstack(LIBMTP_mtpdevice_t *device) {
    if (device == NULL) {
        LIBMTP_ERROR("LIBMTP PANIC: Trying to clear the error stack of a NULL "
                     "device!\n");
    } else {
        LIBMTP_error_t *tmp = device->errorstack;
    ...
    for (j = 0; j < nrofprops; j++) {
        ...
        case PTP_OPC_ObjectSize:
            ...
            if (device->object_bitsize == 64) {
                params->objects[i].oi.ObjectCompressedSize =
        ...
        default: {
            ...
            if (params->objects[i].nrofmtpprops) {
                newprops = realloc(params->objects[i].mtpprops,
                                   (params->objects[i].nrofmtpprops + 1) *
                                       sizeof(MTPProperties));
            } else {
            ...
            memcpy(
                &params->objects[i].mtpprops[params->objects[i].nrofmtpprops],
...
static int sort_storage_by(LIBMTP_mtpdevice_t *device, int const sortby) {
    ...
    while (oldhead != NULL) {
        ...
        while (ptr1 != NULL) {
            ...
            if (sortby == LIBMTP_STORAGE_SORTBY_FREESPACE &&
...
static uint32_t get_writeable_storageid(LIBMTP_mtpdevice_t *device,
    ...
    if (storage == NULL) {
        ...
        while (storage != NULL) {
            ...
            // Also check the access capability to avoid e.g. deletable only
            // storages
...
static int get_suggested_storage_id(LIBMTP_mtpdevice_t *device,
                                    uint64_t fitsize, uint32_t parent_id) {
    PTPParams *params = (PTPParams *)device->params;
    PTPObject *ob;
    uint16_t ret;
    int subcall_ret;
...
void LIBMTP_Dump_Device_Info(LIBMTP_mtpdevice_t *device) {
    ...
    for (i = 0; i < params->deviceinfo.DevicePropertiesSupported_len; i++) {
        char const *propdesc = ptp_get_property_description(
            params, params->deviceinfo.DevicePropertiesSupported[i]);
    ...
    if (ptp_operation_issupported(params, PTP_OC_MTP_GetObjectPropsSupported)) {
        ...
        for (i = 0; i < params->deviceinfo.ImageFormats_len; i++) {
            ...
            ret = ptp_mtp_getobjectpropssupported(
                params, params->deviceinfo.ImageFormats[i], &propcnt, &props);
            if (ret != PTP_RC_OK) {
                add_ptp_error_to_errorstack(
                    device, ret, "LIBMTP_Dump_Device_Info(): "
                ...
                for (j = 0; j < propcnt; j++) {
                    ...
                    ret = ptp_mtp_getobjectpropdesc(
                        params, props[j], params->deviceinfo.ImageFormats[i],
                        &opd);
                    if (ret != PTP_RC_OK) {
                        add_error_to_errorstack(
                            device, LIBMTP_ERROR_GENERAL,
                        ...
                        case PTP_DTC_UINT32:
                            ...
                            case PTP_OPFF_Enumeration:
                                ...
                                if (params->deviceinfo.ImageFormats[i] ==
                                    ...
                                    for (k = 0;
                                         k < opd.FORM.Enum.NumberOfValues;
                                         k++) {
                                        if (opd.FORM.Enum.SupportedValue[k]
                                                .u32 == 0) {
                                            printf("ANY, ");
                                        } else {
                                            char fourcc[6];
                                            fourcc[0] =
                                                (opd.FORM.Enum.SupportedValue[k]
                                                     .u32 >>
                                                 24) &
                                                0xFFU;
                                            fourcc[1] =
...
int LIBMTP_Get_Supported_Filetypes(LIBMTP_mtpdevice_t *device,
    ...
    // This is more memory than needed if there are unknown types, but what the
    // heck.
    localtypes = (uint16_t *)malloc(params->deviceinfo.ImageFormats_len *
                                    sizeof(uint16_t));
    localtypelen = 0;

    for (i = 0; i < params->deviceinfo.ImageFormats_len; i++) {
        uint16_t localtype =
            map_ptp_type_to_libmtp_type(params->deviceinfo.ImageFormats[i]);
...
int LIBMTP_Get_Storage(LIBMTP_mtpdevice_t *device, int const sortby) {
    ...
    if (!ptp_operation_issupported(params, PTP_OC_GetStorageInfo)) {
        for (i = 0; i < storageIDs.n; i++) {
            ...
            storage = (LIBMTP_devicestorage_t *)malloc(
                sizeof(LIBMTP_devicestorage_t));
        ...
        for (i = 0; i < storageIDs.n; i++) {
            uint16_t ret;
            ret =
                ptp_getstorageinfo(params, storageIDs.Storage[i], &storageInfo);
            if (ret != PTP_RC_OK) {
                add_ptp_error_to_errorstack(device, ret,
                                            "LIBMTP_Get_Storage(): "
...
static LIBMTP_file_t *obj2file(LIBMTP_mtpdevice_t *device, PTPObject *ob) {
    ...
    // We only have 32-bit file size here; later we use the PTP_OPC_ObjectSize
    // property
    ...
    if (ob->mtpprops) {
        ...
        ret = ptp_mtp_getobjectpropssupported(
            params, map_libmtp_type_to_ptp_type(file->filetype), &propcnt,
            &props);
        if (ret != PTP_RC_OK) {
            add_ptp_error_to_errorstack(
                device, ret,
                "obj2file: call to ptp_mtp_getobjectpropssupported() failed.");
            ...
            for (i = 0; i < propcnt; i++) {
                ...
                case PTP_OPC_ObjectSize:
                    if (device->object_bitsize == 64) {
                        file->filesize = get_u64_from_object(
                            device, file->item_id, PTP_OPC_ObjectSize, 0);
                    } else {
                        file->filesize = get_u32_from_object(
                            device, file->item_id, PTP_OPC_ObjectSize, 0);
                    }
                    break;
                default:
...
LIBMTP_file_t *LIBMTP_Get_Filelisting(LIBMTP_mtpdevice_t *device) {
    ...
    LIBMTP_INFO("WARNING: please update your code to use "
 ...
 * and/or <code>parent_id</code> field of the returned
 * <code>LIBMTP_file_t</code>
 * struct. To arrange by folders or files you typically have to create the
 * proper
 ...
 */
LIBMTP_file_t *
LIBMTP_Get_Filelisting_With_Callback(LIBMTP_mtpdevice_t *device,
    ...
    for (i = 0; i < params->nrofobjects; i++) {
        ...
        // double progressPercent = (double)i*(double)100.0 /
        // (double)params->handles.n;
...
LIBMTP_file_t *LIBMTP_Get_Files_And_Folders(LIBMTP_mtpdevice_t *device,
    ...
    ret = ptp_getobjecthandles(params, storageid, PTP_GOH_ALL_FORMATS, parent,
                               &currentHandles);
...
/**
 * This function maps and copies a property onto the track metadata if
 * applicable.
 */
static void pick_property_to_track_metadata(LIBMTP_mtpdevice_t *device,
                                            MTPProperties *prop,
...
static void get_track_metadata(LIBMTP_mtpdevice_t *device,
     ...
     * If we have a cached, large set of metadata, then use it!
     */
    ret = ptp_object_want(params, track->item_id, PTPOBJECT_MTPPROPLIST_LOADED,
                          &ob);
    if (ob->mtpprops) {
        prop = ob->mtpprops;
        for (i = 0; i < ob->nrofmtpprops; i++, prop++)
        ...
        // First see which properties can be retrieved for this object format
        ret = ptp_mtp_getobjectpropssupported(
            params, map_libmtp_type_to_ptp_type(track->filetype), &propcnt,
            &props);
        if (ret != PTP_RC_OK) {
            add_ptp_error_to_errorstack(
                device, ret, "get_track_metadata(): call to "
                             "ptp_mtp_getobjectpropssupported() failed.");
            ...
            for (i = 0; i < propcnt; i++) {
                switch (props[i]) {
                case PTP_OPC_Name:
                    track->title = get_string_from_object(
                        device, track->item_id, PTP_OPC_Name);
                    break;
                case PTP_OPC_Artist:
                    track->artist = get_string_from_object(
                        device, track->item_id, PTP_OPC_Artist);
                    break;
                case PTP_OPC_Composer:
                    track->composer = get_string_from_object(
                        device, track->item_id, PTP_OPC_Composer);
                    break;
                case PTP_OPC_Duration:
                    track->duration = get_u32_from_object(
                        device, track->item_id, PTP_OPC_Duration, 0);
                    break;
                case PTP_OPC_Track:
                    track->tracknumber = get_u16_from_object(
                        device, track->item_id, PTP_OPC_Track, 0);
                    break;
                case PTP_OPC_Genre:
                    track->genre = get_string_from_object(
                        device, track->item_id, PTP_OPC_Genre);
                    break;
                case PTP_OPC_AlbumName:
                    track->album = get_string_from_object(
                        device, track->item_id, PTP_OPC_AlbumName);
                    break;
                case PTP_OPC_OriginalReleaseDate:
                    track->date = get_string_from_object(
                        device, track->item_id, PTP_OPC_OriginalReleaseDate);
                    break;
                // These are, well not so important.
                case PTP_OPC_SampleRate:
                    track->samplerate = get_u32_from_object(
                        device, track->item_id, PTP_OPC_SampleRate, 0);
                    break;
                case PTP_OPC_NumberOfChannels:
                    track->nochannels = get_u16_from_object(
                        device, track->item_id, PTP_OPC_NumberOfChannels, 0);
                    break;
                case PTP_OPC_AudioWAVECodec:
                    track->wavecodec = get_u32_from_object(
                        device, track->item_id, PTP_OPC_AudioWAVECodec, 0);
                    break;
                case PTP_OPC_AudioBitRate:
                    track->bitrate = get_u32_from_object(
                        device, track->item_id, PTP_OPC_AudioBitRate, 0);
                    break;
                case PTP_OPC_BitRateType:
                    track->bitratetype = get_u16_from_object(
                        device, track->item_id, PTP_OPC_BitRateType, 0);
                    break;
                case PTP_OPC_Rating:
                    track->rating = get_u16_from_object(device, track->item_id,
                                                        PTP_OPC_Rating, 0);
                    break;
                case PTP_OPC_UseCount:
                    track->usecount = get_u32_from_object(
                        device, track->item_id, PTP_OPC_UseCount, 0);
                    break;
                case PTP_OPC_ObjectSize:
                    if (device->object_bitsize == 64) {
                        track->filesize = get_u64_from_object(
                            device, track->item_id, PTP_OPC_ObjectSize, 0);
                    } else {
                        track->filesize = (uint64_t)get_u32_from_object(
                            device, track->item_id, PTP_OPC_ObjectSize, 0);
                    }
                    break;
                }
 ...
 */
LIBMTP_track_t *
LIBMTP_Get_Tracklisting_With_Callback(LIBMTP_mtpdevice_t *device,
                                      LIBMTP_progressfunc_t const callback,
                                      void const *const data) {
    return LIBMTP_Get_Tracklisting_With_Callback_For_Storage(device, 0,
                                                             callback, data);
}
 ...
 * LIBMTP_track_t *tracklist;
 *
 * tracklist = LIBMTP_Get_Tracklisting_With_Callback_For_Storage(device,
 * storage_id, callback, data);
 * while (tracklist != NULL) {
 *   LIBMTP_track_t *tmp;
 *
 *   // Do something on each element in the list here...
 ...
 * @see LIBMTP_Get_Trackmetadata()
 */
LIBMTP_track_t *LIBMTP_Get_Tracklisting_With_Callback_For_Storage(
    LIBMTP_mtpdevice_t *device, uint32_t const storage_id,
    ...
    for (i = 0; i < params->nrofobjects; i++) {
        ...
        if (!LIBMTP_FILETYPE_IS_TRACK(mtptype) &&
            // This row lets through undefined files for examination since they
            // may be forgotten OGG files.
...
LIBMTP_track_t *LIBMTP_Get_Trackmetadata(LIBMTP_mtpdevice_t *device,
    ...
    if (!LIBMTP_FILETYPE_IS_TRACK(mtptype) &&
        ...
        (ob->oi.ObjectFormat != PTP_OFC_Undefined ||
         (!FLAG_IRIVER_OGG_ALZHEIMER(ptp_usb) &&
          !FLAG_OGG_IS_UNKNOWN(ptp_usb) && !FLAG_FLAC_IS_UNKNOWN(ptp_usb)))) {
        // printf("Not a music track (name: %s format: %d), skipping...\n",
        // oi->Filename, oi->ObjectFormat);
        return NULL;
    }
     ...
     * for these bugged devices only.
     */
    if (track->filetype == LIBMTP_FILETYPE_UNKNOWN && track->filename != NULL) {
        if ((FLAG_IRIVER_OGG_ALZHEIMER(ptp_usb) ||
             FLAG_OGG_IS_UNKNOWN(ptp_usb)) &&
            has_ogg_extension(track->filename))
            track->filetype = LIBMTP_FILETYPE_OGG;
...
int LIBMTP_Get_File_To_File(LIBMTP_mtpdevice_t *device, uint32_t const id,
    ...
    if (path == NULL) {
        add_error_to_errorstack(
            device, LIBMTP_ERROR_GENERAL,
            "LIBMTP_Get_File_To_File(): Bad arguments, path was NULL.");
        return -1;
    }
...
// Open file
    ...
    if ((fd = _open(path, O_RDWR | O_CREAT | O_TRUNC | O_BINARY, _S_IREAD)) ==
        -1) {
#else
    if ((fd = open(path, O_RDWR | O_CREAT | O_TRUNC | O_BINARY, S_IRWXU)) ==
        -1) {
#endif
#else
    if ((fd = open(path, O_RDWR | O_CREAT | O_TRUNC, S_IRWXU | S_IRGRP)) ==
        -1) {
#endif
        add_error_to_errorstack(
            device, LIBMTP_ERROR_GENERAL,
            "LIBMTP_Get_File_To_File(): Could not create file.");
        return -1;
    }
...
int LIBMTP_Get_File_To_File_Descriptor(LIBMTP_mtpdevice_t *device,
                                       uint32_t const id, int const fd,
                                       LIBMTP_progressfunc_t const callback,
                                       void const *const data) {
    uint16_t ret;
    PTPParams *params = (PTPParams *)device->params;
    ...
    ptp_usb->current_transfer_total =
        ob->oi.ObjectCompressedSize + PTP_USB_BULK_HDR_LEN +
        sizeof(uint32_t); // Request length, one parameter
    ...
    if (ret == PTP_ERROR_CANCEL) {
        add_error_to_errorstack(
            device, LIBMTP_ERROR_CANCELLED,
            "LIBMTP_Get_File_From_File_Descriptor(): Cancelled transfer.");
        return -1;
    }
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(device, ret, "LIBMTP_Get_File_To_File_"
                                                 "Descriptor(): Could not get "
                                                 "file from device.");
        return -1;
    }
...
int LIBMTP_Get_Track_To_Handler(LIBMTP_mtpdevice_t *device, uint32_t const id,
                                ...
                                void const *const data) {
    // This is just a wrapper
    return LIBMTP_Get_File_To_Handler(device, id, put_func, priv, callback,
                                      data);
}
 ...
 *        as the <code>metadata-&gt;filename</code>,
 * <code>metadata-&gt;parent_id</code>
...
int LIBMTP_Send_Track_From_File(LIBMTP_mtpdevice_t *device,
    ...
    if (path == NULL) {
        add_error_to_errorstack(
            device, LIBMTP_ERROR_GENERAL,
            "LIBMTP_Send_Track_From_File(): Bad arguments, path was NULL.");
        return -1;
    }
...
#endif
        LIBMTP_ERROR("LIBMTP_Send_Track_From_File(): Could not open source "
                     "file \"%s\"\n",
                     path);
        return -1;
    }

    ret = LIBMTP_Send_Track_From_File_Descriptor(device, fd, metadata, callback,
                                                 data);
 ...
 */
static int check_filename_exists(PTPParams *params,
                                 char const *const filename) {
    int i;
...
int LIBMTP_Send_Track_From_File_Descriptor(LIBMTP_mtpdevice_t *device,
    ...
    if
        FLAG_UNIQUE_FILENAMES(ptp_usb) {
            filedata.filename =
                generate_unique_filename(params, metadata->filename);
        }
    else {
    ...
    subcall_ret = LIBMTP_Send_File_From_File_Descriptor(device, fd, &filedata,
                                                        callback, data);
    ...
    if (subcall_ret != 0) {
        ...
        // We used to delete the file here, but don't... It might be OK after
        // all.
    ...
    // note we don't need to update the cache here because
    // LIBMTP_Send_File_From_File_Descriptor
    // has added the object handle and LIBMTP_Update_Track_Metadata has added
    // the metadata.
...
int LIBMTP_Send_Track_From_Handler(LIBMTP_mtpdevice_t *device,
    ...
    subcall_ret = LIBMTP_Send_File_From_Handler(device, get_func, priv,
                                                &filedata, callback, data);
 ...
 *        as the <code>filedata-&gt;filename</code>,
 * <code>filedata-&gt;parent_id</code>
...
int LIBMTP_Send_File_From_File(LIBMTP_mtpdevice_t *device,
    ...
    if (path == NULL) {
        add_error_to_errorstack(
            device, LIBMTP_ERROR_GENERAL,
            "LIBMTP_Send_File_From_File(): Bad arguments, path was NULL.");
        return -1;
    }
...
// Open file
    ...
    if ((fd = open(path, O_RDONLY)) == -1) {
        ...
        add_error_to_errorstack(
            device, LIBMTP_ERROR_GENERAL,
            "LIBMTP_Send_File_From_File(): Could not open source file.");
        return -1;
    }

    ret = LIBMTP_Send_File_From_File_Descriptor(device, fd, filedata, callback,
                                                data);
...
int LIBMTP_Send_File_From_File_Descriptor(LIBMTP_mtpdevice_t *device,
    ...
    if (send_file_object_info(device, filedata)) {
        // no need to output an error since send_file_object_info will already
        // have done so
        return -1;
    }
    ...
    // Callbacks
    ptp_usb->callback_active = 1;
    // The callback will deactivate itself after this amount of data has been
    // sent
    // One BULK header for the request, one for the data phase. No parameters to
    // the request.
    ptp_usb->current_transfer_total =
        filedata->filesize + PTP_USB_BULK_HDR_LEN * 2;
    ptp_usb->current_transfer_complete = 0;
    ptp_usb->current_transfer_callback = callback;
    ptp_usb->current_transfer_callback_data = data;
...
static int send_file_object_info(LIBMTP_mtpdevice_t *device,
  ...
  if (filedata->filesize == 0) {
    ...
    if (localph == 0 && use_primary_storage) {
        if (LIBMTP_FILETYPE_IS_AUDIO(filedata->filetype)) {
        ...
        } else if (of == PTP_OFC_EXIF_JPEG || of == PTP_OFC_JP2 ||
                   of == PTP_OFC_JPX || of == PTP_OFC_JFIF ||
                   of == PTP_OFC_TIFF || of == PTP_OFC_TIFF_IT ||
                   of == PTP_OFC_BMP || of == PTP_OFC_GIF ||
                   of == PTP_OFC_PICT || of == PTP_OFC_PNG ||
            ...
            localph = device->default_picture_folder;
                   ...
                   of == PTP_OFC_MTP_vCard2 || of == PTP_OFC_MTP_vCard3 ||
    ...
    if (ptp_operation_issupported(params, PTP_OC_MTP_SendObjectPropList) &&
         ...
         *    Param3: 0x00003000 <- file type PTP_OFC_Undefined - we don't know
         * about PDF files
         ...
         *    00 00 00 00 00 00 00 03 DC 04 00 00 00 00 00 00 - dc03 =
         * protection status
         *    00 4F DC 02 00 01                               - dc4f = non
         * consumable
         *    Length: 0x00000046
         ...
         *    0x00000000 <- Object handle, set to 0x00000000 since it is
         * unknown!
         *    0xdc07     <- metadata type: file name
         *    0xffff     <- metadata type: string
         *    0x0d       <- number of (uint16_t) characters
         *    4b 53 30 36 30 33 30 36 2e 50 64 66 00 "KS060306.pdf", null
         * terminated
         *    0x00000000 <- Object handle, set to 0x00000000 since it is
         * unknown!
         ...
         *    ... Then update metadata one-by one, actually (instead of sending
         * it first!) ...
         */
        MTPProperties *props = NULL;
        int nrofprops = 0;
        ...
        ret =
            ptp_mtp_getobjectpropssupported(params, of, &propcnt, &properties);
        ...
        for (i = 0; i < propcnt; i++) {
            ...
            if (ret != PTP_RC_OK) {
                add_ptp_error_to_errorstack(
                    device, ret, "send_file_object_info(): "
                                 "could not get property description.");
            } else if (opd.GetSet) {
                switch (properties[i]) {
                case PTP_OPC_ObjectFileName:
                ...
                case PTP_OPC_NonConsumable:
                    ...
                    prop->propval.u8 =
                        0x00; /* It is supported, then it is consumable */
                    break;
                case PTP_OPC_Name:
                    prop = ptp_get_new_object_prop_entry(&props, &nrofprops);
                    prop->ObjectHandle = filedata->item_id;
                ...
                case PTP_OPC_DateModified:
                    ...
                    if (!FLAG_CANNOT_HANDLE_DATEMODIFIED(ptp_usb)) {
                        prop =
                            ptp_get_new_object_prop_entry(&props, &nrofprops);
                        prop->ObjectHandle = filedata->item_id;
        ...
        ret = ptp_mtp_sendobjectproplist(params, &store, &localph,
                                         &filedata->item_id, of,
                                         filedata->filesize, props, nrofprops);
...
int LIBMTP_Update_Track_Metadata(LIBMTP_mtpdevice_t *device,
    ...
    // First see which properties can be set on this file format and apply
    // accordingly
    // i.e only try to update this metadata for object tags that exist on the
    // current player.
    ret = ptp_mtp_getobjectpropssupported(
        params, map_libmtp_type_to_ptp_type(metadata->filetype), &propcnt,
        &properties);
    if (ret != PTP_RC_OK) {
        // Just bail out for now, nothing is ever set.
        add_error_to_errorstack(
    ...
    if (ptp_operation_issupported(params, PTP_OC_MTP_SetObjPropList) &&
        ...
        for (i = 0; i < propcnt; i++) {
            ...
            ret = ptp_mtp_getobjectpropdesc(
                params, properties[i],
                map_libmtp_type_to_ptp_type(metadata->filetype), &opd);
            if (ret != PTP_RC_OK) {
                add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                        "LIBMTP_Update_Track_Metadata(): "
                                        "could not get property description.");
        ...
        for (i = 0; i < propcnt; i++) {
            ...
            if (ret != PTP_RC_OK) {
                ...
                case PTP_OPC_Name:
                    // Update title
                    ret = set_object_string(device, metadata->item_id,
                                            PTP_OPC_Name, metadata->title);
                    if (ret != 0) {
                        add_error_to_errorstack(
                            device, LIBMTP_ERROR_GENERAL,
                            "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_AlbumName:
                    // Update album
                    ret = set_object_string(device, metadata->item_id,
                                            PTP_OPC_AlbumName, metadata->album);
                    if (ret != 0) {
                        add_error_to_errorstack(
                            device, LIBMTP_ERROR_GENERAL,
                            "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_Artist:
                    // Update artist
                    ret = set_object_string(device, metadata->item_id,
                                            PTP_OPC_Artist, metadata->artist);
                    if (ret != 0) {
                        add_error_to_errorstack(
                            device, LIBMTP_ERROR_GENERAL,
                            "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_Composer:
                    // Update composer
                    ret =
                        set_object_string(device, metadata->item_id,
                                          PTP_OPC_Composer, metadata->composer);
                    if (ret != 0) {
                        add_error_to_errorstack(
                            device, LIBMTP_ERROR_GENERAL,
                            "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_Genre:
                    ...
                    if (metadata->genre) {
                        ret = set_object_string(device, metadata->item_id,
                                                PTP_OPC_Genre, metadata->genre);
                        if (ret != 0) {
                            add_error_to_errorstack(device,
                                                    LIBMTP_ERROR_GENERAL,
                                                    "update_abstract_list(): "
                                                    "could not set genre.");
                        }
                    }
                    break;
                case PTP_OPC_Duration:
                    ...
                    if (metadata->duration != 0) {
                        ret = set_object_u32(
                            device, metadata->item_id, PTP_OPC_Duration,
                            adjust_u32(metadata->duration, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_Track:
                    ...
                    if (metadata->tracknumber != 0) {
                        ret = set_object_u16(
                            device, metadata->item_id, PTP_OPC_Track,
                            adjust_u16(metadata->tracknumber, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_OriginalReleaseDate:
                    ...
                    // The date can be zero, but some devices do not support
                    // setting zero
                    // dates (and it seems that a zero date should never be set
                    // anyway)
                    if (metadata->date) {
                        ret = set_object_string(device, metadata->item_id,
                                                PTP_OPC_OriginalReleaseDate,
                ...
                case PTP_OPC_SampleRate:
                    ...
                    if (metadata->samplerate != 0) {
                        ret = set_object_u32(
                            device, metadata->item_id, PTP_OPC_SampleRate,
                            adjust_u32(metadata->samplerate, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_NumberOfChannels:
                    ...
                    if (metadata->nochannels != 0) {
                        ret = set_object_u16(
                            device, metadata->item_id, PTP_OPC_NumberOfChannels,
                            adjust_u16(metadata->nochannels, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_AudioWAVECodec:
                    ...
                    if (metadata->wavecodec != 0) {
                        ret = set_object_u32(
                            device, metadata->item_id, PTP_OPC_AudioWAVECodec,
                            adjust_u32(metadata->wavecodec, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_AudioBitRate:
                    ...
                    if (metadata->bitrate != 0) {
                        ret = set_object_u32(
                            device, metadata->item_id, PTP_OPC_AudioBitRate,
                            adjust_u32(metadata->bitrate, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_BitRateType:
                    ...
                    if (metadata->bitratetype != 0) {
                        ret = set_object_u16(
                            device, metadata->item_id, PTP_OPC_BitRateType,
                            adjust_u16(metadata->bitratetype, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_Rating:
                    ...
                    if (metadata->rating != 0) {
                        ret = set_object_u16(
                            device, metadata->item_id, PTP_OPC_Rating,
                            adjust_u16(metadata->rating, &opd));
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                ...
                case PTP_OPC_UseCount:
                    ...
                    ret = set_object_u32(device, metadata->item_id,
                                         PTP_OPC_UseCount,
                                         adjust_u32(metadata->usecount, &opd));
                    if (ret != 0) {
                        add_error_to_errorstack(
                            device, LIBMTP_ERROR_GENERAL,
                            "LIBMTP_Update_Track_Metadata(): "
                ...
                // NOTE: File size is not updated, this should not change
                // anyway.
                // neither will we change the filename.
                default:
                    break;
        ...
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Update_Track_Metadata(): "
                                "Your device doesn't seem to support any known "
...
int LIBMTP_Delete_Object(LIBMTP_mtpdevice_t *device, uint32_t object_id) {
    ...
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(
            device, ret, "LIBMTP_Delete_Object(): could not delete object.");
        return -1;
    }
 ...
 */
static int set_object_filename(LIBMTP_mtpdevice_t *device, uint32_t object_id,
                               uint16_t ptp_type, const char **newname_ptr) {
    PTPParams *params = (PTPParams *)device->params;
    PTP_USB *ptp_usb = (PTP_USB *)device->usbinfo;
    PTPObjectPropDesc opd;
    uint16_t ret;
    ...
    ret = ptp_mtp_getobjectpropdesc(params, PTP_OPC_ObjectFileName, ptp_type,
                                    &opd);
    if (ret != PTP_RC_OK) {
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                "set_object_filename(): "
    ...
    if (ptp_operation_issupported(params, PTP_OC_MTP_SetObjPropList) &&
        ...
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                "set_object_filename(): "
                                " your device doesn't seem to support any "
                                "known way of setting metadata.");
        ptp_free_objectpropdesc(&opd);
        return -1;
    }
 ...
 *        this name can be different than newname depending of device
 * restrictions.
 ...
 * @return 0 on success, any other value means failure.
 */
int LIBMTP_Set_Folder_Name(LIBMTP_mtpdevice_t *device, LIBMTP_folder_t *folder,
                           const char *newname) {
    int ret;

    ret = set_object_filename(device, folder->folder_id, PTP_OFC_Association,
                              &newname);
...
int LIBMTP_Set_Playlist_Name(LIBMTP_mtpdevice_t *device,
    ...
    ret = set_object_filename(device, playlist->playlist_id,
                              PTP_OFC_MTP_AbstractAudioVideoPlaylist, &newname);
 ...
 */
int LIBMTP_Set_Object_Filename(LIBMTP_mtpdevice_t *device, uint32_t object_id,
                               char *newname) {
    int ret;
    LIBMTP_file_t *file;
...
LIBMTP_folder_t *LIBMTP_Get_Folder_List_For_Storage(LIBMTP_mtpdevice_t *device,
    ...
    for (i = 0; i < params->nrofobjects; i++) {
        ...
        folder->name =
            (ob->oi.Filename) ? (char *)strdup(ob->oi.Filename) : NULL;
...
uint32_t LIBMTP_Create_Folder(LIBMTP_mtpdevice_t *device, char *name,
    ...
    if (!(params->device_flags & DEVICE_FLAG_BROKEN_SEND_OBJECT_PROPLIST) &&
        ...
        MTPProperties *props =
            (MTPProperties *)calloc(2, sizeof(MTPProperties));
    ...
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(
            device, ret, "LIBMTP_Create_Folder: Could not send object info.");
        if (ret == PTP_RC_AccessDenied) {
            add_ptp_error_to_errorstack(device, ret, "ACCESS DENIED.");
        }
        return 0;
    }
    // NOTE: don't destroy the new_folder objectinfo, because it is statically
    // referencing
    // several strings.
...
LIBMTP_playlist_t *LIBMTP_new_playlist_t(void) {
    LIBMTP_playlist_t *new =
        (LIBMTP_playlist_t *)malloc(sizeof(LIBMTP_playlist_t));
    if (new == NULL) {
        return NULL;
    }
    new->playlist_id = 0;
...
LIBMTP_playlist_t *LIBMTP_Get_Playlist_List(LIBMTP_mtpdevice_t *device) {
    ...
    for (i = 0; i < params->nrofobjects; i++) {
        ...
        // For Samsung players we must look for the .spl extension explicitly
        // since
        ...
        if (REQ_SPL && is_spl_playlist(&ob->oi)) {
            ...
            ret = ptp_mtp_getobjectreferences(params, pl->playlist_id,
                                              &pl->tracks, &pl->no_tracks);
            if (ret != PTP_RC_OK) {
                add_ptp_error_to_errorstack(device, ret,
...
LIBMTP_playlist_t *LIBMTP_Get_Playlist(LIBMTP_mtpdevice_t *device,
    ...
    // Then get the track listing for this playlist
    ret = ptp_mtp_getobjectreferences(params, pl->playlist_id, &pl->tracks,
                                      &pl->no_tracks);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(
            device, ret,
...
create_new_abstract_list(LIBMTP_mtpdevice_t *device, char const *const name,
                         ...
                         uint32_t const storageid, uint16_t const objectformat,
                         char const *const suffix, uint32_t *const newid,
                         uint32_t const *const tracks, uint32_t const no_tracks)

{
    ...
    if (!name) {
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                "create_new_abstract_list(): list name was "
                                "NULL, using default name \"Unknown\"");
        return -1;
    }
    ...
    if (!supported) {
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                "create_new_abstract_list(): player does not "
                                "support this abstract type");
    ...
    if (ptp_operation_issupported(params, PTP_OC_MTP_SendObjectPropList) &&
        ...
        *newid = 0x00000000U;
        ...
        ret = ptp_mtp_getobjectpropssupported(params, objectformat, &propcnt,
                                              &properties);

        for (i = 0; i < propcnt; i++) {
            PTPObjectPropDesc opd;

            ret = ptp_mtp_getobjectpropdesc(params, properties[i], objectformat,
                                            &opd);
            if (ret != PTP_RC_OK) {
                add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
        ...
        if (ret != PTP_RC_OK) {
            add_ptp_error_to_errorstack(device, ret, "create_new_abstract_list("
                                                     "): Could not send object "
                                                     "property list.");
            if (ret == PTP_RC_AccessDenied) {
                add_ptp_error_to_errorstack(device, ret, "ACCESS DENIED.");
        ...
        if (ret != PTP_RC_OK) {
            add_ptp_error_to_errorstack(
                device, ret, "create_new_abstract_list(): Could not send "
                             "object info (the playlist itself).");
            if (ret == PTP_RC_AccessDenied) {
                add_ptp_error_to_errorstack(device, ret, "ACCESS DENIED.");
            }
            return -1;
        }
// NOTE: don't destroy new_object objectinfo afterwards - the strings it
// contains are
// not copies.
      ...
      return -1;
        ...
        for (i = 0; i < propcnt; i++) {
            ...
            if (ret != PTP_RC_OK) {
                ...
                case PTP_OPC_Name:
                    if (name != NULL) {
                        ret = set_object_string(device, *newid, PTP_OPC_Name,
                                                name);
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "create_new_abstract_list(): could not set "
                                "entity name.");
                            return -1;
                        }
                    }
                ...
                case PTP_OPC_AlbumArtist:
                    if (artist != NULL) {
                        ret = set_object_string(device, *newid,
                                                PTP_OPC_AlbumArtist, artist);
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                                "create_new_abstract_list(): could not set "
                ...
                case PTP_OPC_Artist:
                    if (artist != NULL) {
                        ret = set_object_string(device, *newid, PTP_OPC_Artist,
                                                artist);
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
                ...
                case PTP_OPC_Genre:
                    if (genre != NULL) {
                        ret = set_object_string(device, *newid, PTP_OPC_Genre,
                                                genre);
                        if (ret != 0) {
                            add_error_to_errorstack(
                                device, LIBMTP_ERROR_GENERAL,
    ...
    if (no_tracks > 0) {
        ...
        ret = ptp_mtp_setobjectreferences(params, *newid, (uint32_t *)tracks,
                                          no_tracks);
        if (ret != PTP_RC_OK) {
            add_ptp_error_to_errorstack(device, ret, "create_new_abstract_list("
...
update_abstract_list(LIBMTP_mtpdevice_t *device, char const *const name,
    ...
    if (ptp_operation_issupported(params, PTP_OC_MTP_SetObjPropList) &&
        ...
        for (i = 0; i < propcnt; i++) {
            ...
            case PTP_OPC_Name:
                // Update title
                ret =
                    set_object_string(device, objecthandle, PTP_OPC_Name, name);
                if (ret != 0) {
                    add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                            "update_abstract_list(): "
                                            "could not set title.");
            ...
            case PTP_OPC_AlbumArtist:
                // Update album artist
                ret = set_object_string(device, objecthandle,
                                        PTP_OPC_AlbumArtist, artist);
                if (ret != 0) {
                    add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                            "update_abstract_list(): "
            ...
            case PTP_OPC_Artist:
                // Update artist
                ret = set_object_string(device, objecthandle, PTP_OPC_Artist,
                                        artist);
                if (ret != 0) {
                    add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                            "update_abstract_list(): "
            ...
            case PTP_OPC_Composer:
                // Update composer
                ret = set_object_string(device, objecthandle, PTP_OPC_Composer,
                                        composer);
                if (ret != 0) {
                    add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                            "update_abstract_list(): "
            ...
            case PTP_OPC_Genre:
                // Update genre (but only if valid)
                if (genre) {
                    ret = set_object_string(device, objecthandle, PTP_OPC_Genre,
                                            genre);
                    if (ret != 0) {
                        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                                "update_abstract_list(): "
    ...
    ret = ptp_mtp_setobjectreferences(params, objecthandle, (uint32_t *)tracks,
                                      no_tracks);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(device, ret, "update_abstract_list(): "
                                                 "could not add tracks as "
                                                 "object references.");
        free(properties);
        return -1;
    }
...
int LIBMTP_Create_New_Playlist(LIBMTP_mtpdevice_t *device,
    ...
    return create_new_abstract_list(
        device, metadata->name, NULL, NULL, NULL, localph, metadata->storage_id,
        PTP_OFC_MTP_AbstractAudioVideoPlaylist, get_playlist_extension(ptp_usb),
        &metadata->playlist_id, metadata->tracks, metadata->no_tracks);
}
...
int LIBMTP_Update_Playlist(LIBMTP_mtpdevice_t *device,
    ...
    return update_abstract_list(device, metadata->name, NULL, NULL, NULL,
                                metadata->playlist_id,
                                PTP_OFC_MTP_AbstractAudioVideoPlaylist,
                                metadata->tracks, metadata->no_tracks);
}
...
/**
 * This function maps and copies a property onto the album metadata if
 * applicable.
 */
...
static void get_album_metadata(LIBMTP_mtpdevice_t *device,
     ...
     * If we have a cached, large set of metadata, then use it!
     */
    ret = ptp_object_want(params, alb->album_id, PTPOBJECT_MTPPROPLIST_LOADED,
                          &ob);
    if (ob->mtpprops) {
        prop = ob->mtpprops;
        for (i = 0; i < ob->nrofmtpprops; i++, prop++)
        ...
        ret = ptp_mtp_getobjectpropssupported(
            params, PTP_OFC_MTP_AbstractAudioAlbum, &propcnt, &props);
        if (ret != PTP_RC_OK) {
            add_ptp_error_to_errorstack(
                device, ret, "get_album_metadata(): call to "
                             "ptp_mtp_getobjectpropssupported() failed.");
            // Just bail out for now, nothing is ever set.
            return;
        } else {
            for (i = 0; i < propcnt; i++) {
                ...
                case PTP_OPC_Name:
                    alb->name =
                        get_string_from_object(device, ob->oid, PTP_OPC_Name);
                    break;
                case PTP_OPC_AlbumArtist:
                    if (alb->artist != NULL)
                        free(alb->artist);
                    alb->artist = get_string_from_object(device, ob->oid,
                                                         PTP_OPC_AlbumArtist);
                    break;
                case PTP_OPC_Artist:
                    if (alb->artist == NULL)
                    ...
                    break;
                case PTP_OPC_Composer:
                    alb->composer = get_string_from_object(device, ob->oid,
                                                           PTP_OPC_Composer);
                    break;
                case PTP_OPC_Genre:
                    alb->genre =
                        get_string_from_object(device, ob->oid, PTP_OPC_Genre);
                    break;
                default:
                    break;
                }
 ...
 * @see LIBMTP_Get_Album()
 */
LIBMTP_album_t *LIBMTP_Get_Album_List_For_Storage(LIBMTP_mtpdevice_t *device,
                                                  uint32_t const storage_id) {
    PTPParams *params = (PTPParams *)device->params;
    ...
    for (i = 0; i < params->nrofobjects; i++) {
        ...
        ret = ptp_mtp_getobjectreferences(params, alb->album_id, &alb->tracks,
                                          &alb->no_tracks);
        if (ret != PTP_RC_OK) {
            add_ptp_error_to_errorstack(
                device, ret,
...
int LIBMTP_Create_New_Album(LIBMTP_mtpdevice_t *device,
    ...
    return create_new_abstract_list(
        device, metadata->name, metadata->artist, metadata->composer,
        metadata->genre, localph, metadata->storage_id,
        PTP_OFC_MTP_AbstractAudioAlbum, ".alb", &metadata->album_id,
        metadata->tracks, metadata->no_tracks);
}
...
LIBMTP_filesampledata_t *LIBMTP_new_filesampledata_t(void) {
    LIBMTP_filesampledata_t *new =
        (LIBMTP_filesampledata_t *)malloc(sizeof(LIBMTP_filesampledata_t));
    if (new == NULL) {
        return NULL;
    }
    new->height = 0;
 ...
 *        destroy this struct with
 * <code>LIBMTP_destroy_filesampledata_t()</code>
 *        after use.
 * @return 0 on success, any other value means failure.
...
int LIBMTP_Get_Representative_Sample_Format(LIBMTP_mtpdevice_t *device,
    ...
    *sample = NULL;
    ...
    ret = ptp_mtp_getobjectpropssupported(
        params, map_libmtp_type_to_ptp_type(filetype), &propcnt, &props);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(device, ret, "LIBMTP_Get_Representative_"
                                                 "Sample_Format(): could not "
    ...
    if (support_data && support_format && support_height && support_width &&
        !support_duration) {
        // Something that supports height and width and not duration is likely
        // to be JPEG
         ...
         */
        ptp_mtp_getobjectpropdesc(params, PTP_OPC_RepresentativeSampleFormat,
                                  map_libmtp_type_to_ptp_type(filetype),
                                  &opd_format);
        retsam->filetype = map_ptp_type_to_libmtp_type(
            opd_format.FORM.Enum.SupportedValue[0].u16);
        ptp_free_objectpropdesc(&opd_format);
        /* Populate the maximum image height */
        ptp_mtp_getobjectpropdesc(params, PTP_OPC_RepresentativeSampleWidth,
                                  map_libmtp_type_to_ptp_type(filetype),
                                  &opd_width);
        retsam->width = opd_width.FORM.Range.MaximumValue.u32;
        ptp_free_objectpropdesc(&opd_width);
        /* Populate the maximum image width */
        ptp_mtp_getobjectpropdesc(params, PTP_OPC_RepresentativeSampleHeight,
                                  map_libmtp_type_to_ptp_type(filetype),
                                  &opd_height);
        ...
        if (support_size) {
            ptp_mtp_getobjectpropdesc(params, PTP_OPC_RepresentativeSampleSize,
                                      map_libmtp_type_to_ptp_type(filetype),
                                      &opd_size);
        ...
        *sample = retsam;
    } else if (support_data && support_format && !support_height &&
               !support_width && support_duration) {
        ...
        ptp_mtp_getobjectpropdesc(params, PTP_OPC_RepresentativeSampleDuration,
                                  map_libmtp_type_to_ptp_type(filetype),
                                  &opd_duration);
...
int LIBMTP_Send_Representative_Sample(LIBMTP_mtpdevice_t *device,
    ...
    // get the file format for the object we're going to send representative
    // data for
    ret = ptp_object_want(params, id, PTPOBJECT_OBJECTINFO_LOADED, &ob);
    if (ret != PTP_RC_OK) {
        add_error_to_errorstack(
    ...
    ret = ptp_mtp_getobjectpropssupported(params, ob->oi.ObjectFormat, &propcnt,
                                          &props);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(device, ret, "LIBMTP_Send_Representative_"
                                                 "Sample(): could not get "
                                                 "object properties.");
        return -1;
    }
    ...
    if (!supported) {
        free(props);
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Send_Representative_Sample(): object "
    ...
    ret = ptp_mtp_setobjectpropvalue(
        params, id, PTP_OPC_RepresentativeSampleData, &propval, PTP_DTC_AUINT8);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(
            device, ret,
            "LIBMTP_Send_Representative_Sample(): could not send sample data.");
        free(propval.a.v);
        return -1;
    }
    free(propval.a.v);
    ...
    case LIBMTP_FILETYPE_PNG:
        if (!FLAG_BROKEN_SET_SAMPLE_DIMENSIONS(ptp_usb)) {
            ...
            set_object_u32(device, id, PTP_OPC_RepresentativeSampleHeight,
                           sampledata->height);
            set_object_u32(device, id, PTP_OPC_RepresentativeSampleWidth,
                           sampledata->width);
        }
        break;
    default:
        // For anything not an image, set the duration and size
        set_object_u32(device, id, PTP_OPC_RepresentativeSampleDuration,
                       sampledata->duration);
        set_object_u32(device, id, PTP_OPC_RepresentativeSampleSize,
                       sampledata->size);
        break;
    }
...
int LIBMTP_Get_Representative_Sample(LIBMTP_mtpdevice_t *device,
    ...
    if (ret != PTP_RC_OK) {
        add_error_to_errorstack(
            device, LIBMTP_ERROR_GENERAL,
            "LIBMTP_Get_Representative_Sample(): could not get object info.");
        return -1;
    }
    ...
    if (!supported) {
        free(props);
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                "LIBMTP_Get_Representative_Sample(): object "
                                "type doesn't support "
                                "RepresentativeSampleData.");
        return -1;
    }
    ...
    // Get the data
    ret = ptp_mtp_getobjectpropvalue(
        params, id, PTP_OPC_RepresentativeSampleData, &propval, PTP_DTC_AUINT8);
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(
            device, ret,
            "LIBMTP_Get_Representative_Sample(): could not get sample data.");
        return -1;
    }
    ...
    sampledata->width =
        get_u32_from_object(device, id, PTP_OPC_RepresentativeSampleWidth, 0);
    sampledata->height =
        get_u32_from_object(device, id, PTP_OPC_RepresentativeSampleHeight, 0);
    sampledata->duration = get_u32_from_object(
        device, id, PTP_OPC_RepresentativeSampleDuration, 0);
    sampledata->filetype = map_ptp_type_to_libmtp_type(
        get_u16_from_object(device, id, PTP_OPC_RepresentativeSampleFormat,
    ...
    if (!ptp_operation_issupported(params, PTP_OC_ANDROID_GetPartialObject64)) {
        if (!ptp_operation_issupported(params, PTP_OC_GetPartialObject)) {
            add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                    ...
                                    "PTP_OC_GetPartialObject not supported");
            return -1;
        }
        ...
        if (offset >> 32 != 0) {
            add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                    "LIBMTP_GetPartialObject: "
                                    "PTP_OC_GetPartialObject only supports "
                                    "32bit offsets");
            return -1;
        }

        ret = ptp_getpartialobject(params, id, (uint32_t)offset, maxbytes, data,
                                   size);
    } else {
        ret = ptp_android_getpartialobject64(params, id, offset, maxbytes, data,
                                             size);
    }
    if (ret == PTP_RC_OK)
        return 0;
...
int LIBMTP_SendPartialObject(LIBMTP_mtpdevice_t *device, uint32_t const id,
    ...
    if (!ptp_operation_issupported(params, PTP_OC_ANDROID_SendPartialObject)) {
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                ...
                                "PTP_OC_ANDROID_SendPartialObject not "
                                "supported");
        return -1;
    }
...
int LIBMTP_BeginEditObject(LIBMTP_mtpdevice_t *device, uint32_t const id) {
    ...
    if (!ptp_operation_issupported(params, PTP_OC_ANDROID_BeginEditObject)) {
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                ...
                                "PTP_OC_ANDROID_BeginEditObject not supported");
        return -1;
    }
...
int LIBMTP_TruncateObject(LIBMTP_mtpdevice_t *device, uint32_t const id,
    ...
    if (!ptp_operation_issupported(params, PTP_OC_ANDROID_TruncateObject)) {
        add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
                                ...
                                "PTP_OC_ANDROID_TruncateObject not supported");
        return -1;
    }
...
int LIBMTP_Update_Album(LIBMTP_mtpdevice_t *device,
    ...
    return update_abstract_list(
        device, metadata->name, metadata->artist, metadata->composer,
        metadata->genre, metadata->album_id, PTP_OFC_MTP_AbstractAudioAlbum,
        metadata->tracks, metadata->no_tracks);
}
 ...
 * ptp.c/ptp.h files.
 */
void ptp_nikon_getptpipguid(unsigned char *guid) { return; }

/**
 * Add an object to cache.
 * @param device the device which may have a cache to which the object should be
 * added.
 * @param object_id the object to add to the cache.
 */
static void add_object_to_cache(LIBMTP_mtpdevice_t *device,
                                uint32_t object_id) {
    PTPParams *params = (PTPParams *)device->params;
    uint16_t ret;
    ...
    if (ret != PTP_RC_OK) {
        add_ptp_error_to_errorstack(
            device, ret, "add_object_to_cache(): couldn't add object to cache");
    }
}
